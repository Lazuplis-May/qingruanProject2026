...
> **v2 修订说明**：第 13 节"架构偏离汇总"已更新为正式决策记录（9 项偏离全部已决策），§12 Group E 标注决策状态。关联设计文档 `docs/2_detailed_design_v4.md` 第13版修订说明。
...
### Group E：pinia-plugin-persistedstate 迁移（已决策：不迁移）

| 编号 | 模块 | 工作量 | 说明 |
|------|------|-------|------|
| E1 | 持久化方案决策文档归档 | 0.1d | **已决策（2026-06-28）**：正式保留 sessionStorage + BroadcastChannel 方案，不引入 pinia-plugin-persistedstate。决策记录已归档至 `docs/2_detailed_design_v4.md` 第13版修订说明。此任务仅需将已有分析格式化输出为独立决策文档（Markdown 格式）以供团队归档。E1 为纯文档产出，不纳入 Phase 开发工期统计，作为 Phase 2 结束后的独立收尾项执行。 |
...
**推荐方向**：**保留当前 sessionStorage + BroadcastChannel 方案**。（**已正式决策，2026-06-28**）
...
> **决策状态**：以下 9 项偏离已通过正式决策（见 `docs/2_detailed_design_v4.md` 第13版修订说明），设计文档已同步更新。决策依据详见再审议框架第 6 轮产出（`a_v6_diag_v1.md` §12 E1 决策分析框架）。

### 偏离 1（已决策：保留 sessionStorage + BroadcastChannel）

以下偏离点需要决策：是否需要对齐设计文档，还是更新设计文档以适应实际实现。

| 偏离项 | 设计 | 实际 | 影响范围 | 决策 | 状态 |
|--------|------|------|---------|------|:----:|
| 存储介质 | localStorage | sessionStorage | authStore, App.vue | 保留 sessionStorage + BroadcastChannel，设计文档 v4 已同步更新 §1.4/1.5.2 | ✅ 已决策 |
| 持久化插件 | pinia-plugin-persistedstate | 手动读写 sessionStorage + BroadcastChannel | authStore, chatStore, main.ts | 移除 pinia-plugin-persistedstate，设计文档 v4 已同步更新 | ✅ 已决策 |
| composable 架构 | 4 个 composables (useApi/useAuth/useSSE/useUI) | 10 个 composables（6 个设计外新增，3 个设计定义缺失） | useAuth.ts, useSSE.ts, useUI.ts 缺失 | 补建缺失的 3 个 composable（Phase 1 A4/A5/A6） | ✅ 已决策 |
| Admin.vue SSE | 复用 chatStore SSE 逻辑 | Admin.vue 自建 SSE 逻辑 | Admin.vue | 切换为调用 chatStore.sendAdminMessage（Phase 2 D3） | ✅ 已决策 |
| Markdown 渲染 | 统一走 useMarkdown composable | 6 页内联 marked+DOMPurify，仅 LifePlan 用 renderMarkdown() | 多个 views | 统一替换为 renderMarkdown()（Phase 1 A7） | ✅ 已决策 |
| 路由守卫 JWT 过期检测 | 设计步骤 2 检查"Token是否存在且未过期" | 仅检查 token 存在性，不检测 exp | router/index.ts:118 | 引入 useAuth composable（Phase 2 B3） | ✅ 已决策 |
| 路由守卫免责声明函数位置 | useUI().hasAcceptedDisclaimer() / showDisclaimer() | 函数直接定义在 router/index.ts | router/index.ts:93-109 | A8最简修复先行，A5完成后移入 useUI | ✅ 已决策 |
| 路由守卫免责声明拒绝行为 | next(false) 保留来源页 | 始终重定向 /home | router/index.ts:137 | A8 修复行为（Phase 1 0.1d），设计文档 v4 已同步更新 | ✅ 已决策 |
| 路由守卫步骤 3/4 顺序 | 步骤3改密→步骤4角色 | 步骤4角色→步骤3改密 | router/index.ts:123,127 | 维持实际顺序，影响极小；设计文档 v4 已记录 | ✅ 已决策 |
| Axios 拦截器初始化方式 | main.ts 调用 setupAxiosInterceptors() 显式注册 | useApi.ts 模块顶层直接定义拦截器（导入即生效） | main.ts, useApi.ts | 维持模块顶层自注册，设计文档 v4 已同步更新 §1.4 | ✅ 已决策 |
| 样式文件路径 | src/styles/variables.css + src/styles/common.css | src/assets/variables.css + src/styles/animations.css | main.ts:5-6 | 维持实际路径，设计文档 v4 已同步更新 §1.4 | ✅ 已决策 |
