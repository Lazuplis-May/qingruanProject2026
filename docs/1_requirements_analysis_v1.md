...
平台采用 SPA 架构，需在以下层面定义路由结构以保证各模块页面的可访问性和深层链接支持。

**路由方案选型**：推荐使用 hash 路由（`/#/path`）。理由：（1）hash 路由天然兼容静态文件托管（Nginx 无需额外配置 URL 重写规则），部署成本最低；（2）iframe 子页面的 URL 使用 hash 路由时与主框架的 URL 结构保持一致，减少路由冲突风险。若项目组倾向于使用 history 路由（`/path`），需在 Nginx 中配置 `try_files` 回退规则，确保刷新子页面时返回正确的 HTML 入口文件。最终选型在概要设计阶段确定。

**各模块 URL 路径映射**：

| 模块 | URL 路径（hash 路由） | 对应的 iframe 子页面 HTML 文件 |
|------|---------------------|-------------------------------|
| 系统首页 | `/#/home` | `/pages/home.html` |
| 医师咨询（医生列表） | `/#/consultation` | `/pages/consultation.html` |
| 医师咨询（对话） | `/#/consultation/doctor/:id` | `/pages/consultation.html`（通过查询参数区分医生） |
| 生活方案 | `/#/life-plan` | `/pages/life-plan.html` |
| 健康资讯（列表） | `/#/news` | `/pages/news.html` |
| 健康资讯（详情） | `/#/news/article/:id` | `/pages/news.html` |
| 个人中心 | `/#/profile` | `/pages/profile.html` |
| 糖尿病风险预测 | `/#/profile/risk` | `/pages/risk.html` |
| 打卡记录与分析 | `/#/profile/punch` | `/pages/punch.html` |
| 健康建议 | `/#/profile/advice` | `/pages/health-advice.html` |
| 智能管理（管理员） | `/#/admin` | `/pages/admin.html` |
| 登录/注册 | `/#/login` | `/pages/login.html` |

**路由守卫逻辑**：主框架在路由切换时检查目标路径是否需要认证——若目标路径为"需登录"或"需 admin"级别（对照 4.10 节访问控制矩阵），且当前无有效 JWT Token 或 Token 已过期，则自动重定向至 `/#/login`，并在登录成功后回跳至原目标路径。iframe 子页面内部也需独立校验登录态——在页面初始化时从 localStorage 读取 Token，若 Token 不存在或已过期且当前页面需要认证，则通过 `postMessage`（AUTH_SYNC 类型）通知主框架跳转至登录页，自身展示登录引导提示。

**iframe 子页面 URL 规范**：每个 iframe 子页面对应一个独立的 HTML 文件，部署在 Nginx 静态文件目录的 `/pages/` 子目录下。子页面的内部子路由（如文章详情、医生对话）通过 URL 查询参数（`?key=value`）或 hash 片段传递，不创建额外的 HTML 文件。子页面在初始化时解析 URL 参数以确定当前展示的具体视图。

以上路由结构为基于 SPA 架构和 iframe 模块划分的推断，具体 URL 路径命名、HTML 文件命名和参数传递方式在概要设计阶段确定。
...
**会话管理**：用户进入医师对话界面时默认创建新会话（不传 conversation_id），Dify 返回新会话 ID 后前端保存至 localStorage。由于平台配置多位不同科室的医生、每位医生拥有独立的 Dify 聊天助手和对话空间，localStorage 按医生 ID 区分存储键（`conversation_doctor_{id}`），使得每位医生的对话上下文独立保持，切换医生后原对话上下文不丢失。历史对话列表通过 Dify 的会话列表 API 获取（系统需提供 GET /api/chat/doctor/:id/conversations 端点，Express 代理层转发至 Dify 会话列表接口），用户可从历史会话列表中选择并恢复对话。会话不设置自动过期，由用户在 UI 中手动删除。
...
**表单数据跨步骤持久化**：由于平台采用 iframe 架构，风险预测子页面以独立 HTML 文件部署，JavaScript 内存中的表单数据在以下场景可能丢失：（1）用户在步骤间切换时 iframe 意外刷新或重载；（2）用户离开风险预测页面后再返回。为保证"已填写数据不会因步骤切换而丢失"的承诺，推荐使用 `sessionStorage` 作为表单数据的跨步骤持久化存储。具体策略：
- 每完成一步，表单数据序列化为 JSON 并写入 `sessionStorage` 的专用键（如 `risk_form_data`）。
- 每进入新步骤时，优先从 `sessionStorage` 读取已保存数据以回填表单字段。
- 用户提交成功或点击"重新填写"按钮时清除 `sessionStorage` 中的表单数据。
- 风险预测 iframe 子页面刷新或重载时，`sessionStorage` 中的数据仍然保留（`sessionStorage` 在同一浏览器标签页的同一域名下跨页面导航保持，但关闭标签页后清除），使得页面恢复后可继续填写。

此机制依赖所有 iframe 子页面与主框架部署在同一域名下（`sessionStorage` 的同源约束），已在 1.1 节通信机制中作为前提条件明确。具体实现细节（键名规范、序列化格式、异常情况下的回退策略）在概要设计阶段确定。
...
**iframe 子页面的登录态共享**：各 iframe 子页面与主框架部署在同一域名下（受同源策略约束），子页面从 localStorage 读取同一 Token 键值，在 API 请求中携带相同 Authorization 头。主框架在登录/登出时通过 `postMessage` 广播 Token 变更事件，各 iframe 监听后同步更新自身的登录状态。具体实现方式见 1.1 节。
...
- **前端 HTTP 拦截器**：所有 API 请求（fetch）统一经过响应拦截器处理。当收到 401 状态码且错误码为 `AUTH_REQUIRED` 时，拦截器执行以下逻辑：（1）清除 localStorage 中已过期的 Token；（2）通过 `postMessage`（AUTH_SYNC 类型，token=null）广播登出事件至所有 iframe 子页面；（3）展示非阻断性提示条（如"登录已过期，请重新登录"，Toast 或顶部横幅形式，不阻塞用户继续浏览当前页面上的已有内容）；（4）提供"重新登录"按钮，点击后跳转至登录页，登录成功后回跳至当前页面（通过 URL 中的 redirect 参数记录来源路径）。
- **对话中 Token 过期**（SSE 流式接口）：Token 在对话进行中过期时，Dify 代理端点的 JWT 认证中间件返回 401 错误（而非 SSE 流内的错误事件），前端 fetch 请求的响应状态码为 401，触发上述拦截器逻辑。当前对话窗口保持打开状态，已接收的消息内容不丢失，用户重新登录后可继续对话（需重新创建会话或恢复历史会话）。
- **多步骤表单中途过期**：风险预测的多步骤表单数据已通过 `sessionStorage` 持久化（见 4.4 节），Token 过期触发重新登录后，用户返回表单页面时 `sessionStorage` 中的数据仍然保留，表单状态不受 Token 过期影响。
- **iframe 间 Token 过期同步**：主框架拦截到 401 响应后广播 AUTH_SYNC 登出事件，各 iframe 子页面监听该事件后清除自身对 Token 的引用并更新 UI 状态（如隐藏需登录的功能入口）。

Token 过期拦截器的具体实现位置（主框架公共 JS 模块）和提示条 UI 样式在概要设计阶段确定。
...
- **可移植性**：平台基于 Web 标准技术（HTML/CSS/JavaScript）构建，天然支持跨操作系统运行——可在不同 Windows、Linux、macOS 服务器环境之间迁移部署，无需修改源代码（基于 Web 技术栈特性的合理推断）。
...
- 前端构建方式：采用原生 HTML/CSS/JavaScript 开发，不依赖前端构建工具（如 Webpack、Vite）。第三方库（如 marked.js 用于 Markdown 渲染、Swiper 用于轮播组件）通过 CDN 引入或下载后存放于 `/static/lib/` 目录。此选择基于以下推断：（1）实训项目的技术栈要求为 HTML/CSS/JS，未涉及 Node.js 前端工程化工具链；（2）原生开发方式降低项目环境搭建复杂度，成员无需学习构建工具配置；（3）第三方库数量有限（预计 2-3 个），手动管理版本可接受。若项目组在开发阶段发现需要更多第三方依赖或模块化需求，可引入 Vite 作为轻量构建工具。
- 前端源代码推荐目录结构：`/src/pages/`（各模块 HTML 文件，对应 1.2 节路由映射）、`/src/css/`（全局样式和模块样式）、`/src/js/`（公共 JS 模块——API 请求封装、JWT 拦截器、postMessage 通信工具、工具函数）、`/src/js/pages/`（各模块页面 JS 逻辑）、`/static/lib/`（第三方库文件）、`/static/images/`（预置图片资源）、`/static/uploads/`（用户上传文件）。具体目录命名和文件划分在概要设计阶段确定。
...
此外，项目代码需在太乙平台注册并绑定 Gitee/GitHub 进行开源发布，全体成员参与代码提交。开源发布需提交以下三项交付物：（1）**源代码打包**——完整的项目源代码压缩包，含前端 HTML/CSS/JS、Express 后端、数据库初始化脚本；（2）**项目文档**——含需求文档、概要设计说明书、部署说明、API 接口文档等；（3）**开源仓库地址**——Gitee 和/或 GitHub 仓库链接，确保仓库为公开状态且包含完整的提交历史。
