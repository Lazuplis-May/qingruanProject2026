...
本项目是2026年青软实训（东北大学软件学院）的核心实训项目，目标是构建一个面向普通人群和糖尿病患者的**一站式糖尿病预防与健康管理 Web 平台**。平台以 Vue 3 + TypeScript + Vite 为前端技术栈，后端借助 DeepSeek 大模型和 Dify 智能体开发平台提供 AI 能力，帮助用户完成糖尿病风险自评、个性化生活方案获取、在线医师咨询、健康资讯浏览与收藏、日常打卡记录与分析等核心任务。
...
平台以**移动端优先**的 Web 应用形态交付，采用 Vue3 单页应用（SPA）架构，通过底部 Tab 栏切换各功能模块，所有模块运行在同一 Vue 运行时中，不使用 iframe 架构。
...
const routes: RouteRecordRaw[] = [
  ...
  {
    path: '/life-plan',
    component: () => import('@/views/LifePlanView.vue'),
    meta: { requiresAuth: true },
  },
  {
    path: '/profile',
    ...
    children: [
      ...
      {
        path: 'advice',
        component: () => import('@/views/HealthAdviceView.vue'),
        meta: { requiresAuth: true },
      },
    ],
  },
...
**SSE 连接管理**：用户与医生 A 建立 SSE 连接进行流式对话期间，若切换至医生 B 的对话界面，前端应立即关闭医生 A 的 SSE 连接（调用 AbortController.abort()），停止消费医生 A 的流式数据，避免无效连接占用浏览器网络资源和 Express 代理层连接数。仅当前可见的对话窗口维持活跃 SSE 连接——后台（非当前可见）医生的对话不维持 SSE 连接，新消息不会主动推送；用户切回医生 A 时，前端通过历史会话列表恢复对话（通过 GET /api/chat/doctor/:id/conversations 获取历史会话列表后选择并恢复），消息内容通过 Dify 会话 API 按需拉取。前端应限制同时活跃的 SSE 连接数上限为 1（即仅当前可见对话维持连接），此策略基于以下推断：（1）移动端浏览器对并发 HTTP 连接数有严格限制（通常 6 个），维持多个 SSE 长连接可能影响其他 API 请求的及时性；（2）后台对话不维持连接符合移动端优先的资源节约原则；（3）历史消息按需拉取避免不必要的网络流量。此设计为基于 Web 实时通信通用实践的推断，具体 AbortController 清理时机和错误重连策略在概要设计阶段确定。

**验收标准**：
- 平台至少配置 3 位不同科室/领域的医生，每位医生可独立进行多轮对话。
- 用户发送消息后 3 秒内开始收到流式回复（计时起点为用户点击发送按钮的时刻，终点为首个 SSE 事件 data 字段到达前端的时刻；此指标为合理预期而非硬性约束，实际值可能受 AI 服务负载和网络条件影响，详见 7.2 节）。
...
1. **病史状态选择**：用户选择自己的糖尿病病史状态（枚举值：`"healthy"` 健康、`"prediabetes"` 糖尿病前期、`"diagnosed"` 已确诊；其他值将被服务端拒绝），若已确诊则需进一步选择糖尿病类型（枚举值：`"type1"` 1型、`"type2"` 2型、`"gestational"` 妊娠期、`"other"` 其他特殊类型）。
2. **健康信息采集**：填写年龄、性别、身高、体重、腰围、收缩压、家族糖尿病史、是否妊娠（仅女性显示）等关键指标（均来自《中国2型糖尿病防治指南（2020版）》的风险评分因素）。若用户未填写腰围或收缩压，系统基于以下公式估算替代值：
   ...
   **"未提供"判定规则**：腰围和收缩压的"未提供"判定统一采用以下规则——字段值为 `null`、`undefined` 或空字符串时视为"未提供"，触发服务端估算逻辑；字段值为数值 `0` 时视为无效输入，前端表单校验拦截并提示用户修正为有效值或留空（前端输入框 placeholder 提示"留空将自动估算"）。服务端 POST /api/risk/predict 参数校验在 waist/systolic_bp 为 0 值时返回 VALIDATION_ERROR，提示"腰围/收缩压不能为 0，请填写有效值或留空"。此规则基于 Web 表单通用校验实践推断，具体错误码和提示文案在概要设计阶段确定。
...
- 用户完成所有步骤并提交后，10 秒内展示风险等级和对应的预防/管理建议（计时起点为用户点击提交按钮的时刻，终点为风险等级和预防/管理建议内容渲染完成的时刻；此指标为合理预期而非硬性约束，实际值受 Dify 工作流执行时间和 DeepSeek API 响应时间影响，详见 7.2 节）。
...
**方案调整**：用户可以对已生成方案提出修改意见，系统根据反馈重新生成方案。调整后的新方案替换当前活跃方案——旧方案不执行物理删除，而是通过"逻辑过期"机制标记为非活跃（life_plans 表新增 `is_active` 字段，BOOLEAN，默认 TRUE，方案调整时将旧方案记录更新为 `is_active = FALSE`，新生成的方案记录 `is_active = TRUE`）。此策略的必要性在于：punch_in 表通过 plan_id 外键关联到 life_plans 表的具体方案项——若采用物理删除旧方案记录，关联的打卡记录 plan_id 将悬空，导致打卡分析功能失去依从性分析的数据基础。采用逻辑过期后：（1）打卡分析默认仅统计活跃方案项（is_active = TRUE）的打卡数据；（2）历史分析（如趋势回顾）可包含非活跃方案的打卡记录，通过 plan_id 回溯至当时的具体方案项内容。此设计为基于数据关系完整性约束的推断，life_plans 表的 is_active 字段定义和打卡分析的 active/inactive 筛选逻辑在概要设计阶段确定。
...
**验收标准**：
- AI 生成方案至少包含 4 条饮食项（覆盖早/中/晚/加餐）和 3 条运动项（覆盖晨间/晚间/周末），方案内容在 15 秒内开始返回（计时起点为用户点击生成方案的时刻，终点为第一条方案项内容渲染完成的时刻；此指标为合理预期而非硬性约束，实际值受 Dify 工作流执行时间和 DeepSeek API 响应时间影响，详见 7.2 节）。
- 用户在方案页对任意方案项点击打卡后，打卡状态即时更新（1 秒内反馈），且该打卡记录可追溯对应的方案项。

### 4.6 健康资讯

系统通过 Dify 工作流自动生成健康资讯文章，文章内容由 DeepSeek 模型生成。工作流分两阶段运行：
1. **分类生成**：基于用户健康信息生成4个推荐资讯分类标签（饮食指导、运动指南、生活习惯、糖尿病知识科普，基于项目知识库中5份糖尿病 docx 文档的内容范畴推断）。用户健康信息的数据来源优先级为：（1）优先取 user_risk_info 表中该用户最新一条记录中的年龄、性别、身高和体重（用于计算 BMI）、家族史、糖尿病类型等字段；（2）若 user_risk_info 表无记录，降级取 users 表中的注册基本信息（年龄、性别等如有存储）；（3）若均无有效健康数据，降级为通用分类模式——不依赖个人数据，生成面向普通人群的通用资讯分类标签。
2. **文章生成**：用户选择一个分类标签后，系统生成完整的文章（标题、标签列表、正文内容）。

文章正文以 Markdown 格式存储（articles 表的"正文"字段为 Markdown 文本），前端渲染时使用 Markdown 解析器（如 marked）将 Markdown 转换为 HTML 展示。选择 Markdown 格式的理由：（1）Dify 工作流中 LLM 输出 Markdown 是自然且低成本的格式；（2）Markdown 支持标题层级、列表、加粗等排版需求，且前端渲染成熟；（3）纯文本表达能力不足，HTML 对 LLM 生成来说标记噪声过高。此格式选择为基于技术合理性的推断，可在开发阶段根据 Dify 工作流实际输出格式调整。

用户可浏览推荐资讯列表、查看文章详情、将文章加入收藏夹、从收藏夹取消收藏、查看已收藏文章列表。

**验收标准**：
- 用户选择分类标签后，系统在 15 秒内生成并展示完整的文章（含标题、标签、正文）（计时起点为用户点击分类标签的时刻，终点为文章正文渲染完成的时刻；此指标为合理预期而非硬性约束，实际值受 Dify 工作流执行时间和 DeepSeek API 响应时间影响，详见 7.2 节）。
...
- 助手对话窗口从任意页面可通过 FAB 一键唤起，唤起时间不超过 1 秒（计时起点为用户点击 FAB 按钮的时刻，终点为对话窗口渲染完成并展示输入区域的时刻；此指标为前端渲染性能指标，不涉及网络请求）。
...
**验收标准**：
- 管理员通过自然语言完成一次完整 CRUD 操作（如"新增一位医生，姓名张三，科室内分泌科"），全流程在 10 秒内完成并返回操作结果（计时起点为管理员发送消息的时刻，终点为操作结果反馈在对话界面中渲染完成的时刻；此指标为合理预期而非硬性约束，实际值受 Dify Agent 推理时间、Text2SQL 执行时间和 Express 响应时间影响，详见 7.2 节）。
...
export const useAuthStore = defineStore('auth', () => {
  ...
  function setAuth(newToken: string, newRole: string, user: UserInfo): void {
  ...
  return { token, role, userInfo, isLoggedIn, isAdmin, setAuth, clearAuth }
})
```

**Pinia chatStore 类型定义**：chatStore 用于管理医师对话和 AI 助手的会话状态，按医生 ID 维护 conversation_id 映射，同时持久化至 localStorage。以下为概要设计阶段确定完整字段的参考定义：
...
export const useChatStore = defineStore('chat', () => {
  // 医师对话：按医生 ID 维护 conversation_id 映射
  const doctorConversations = ref<Map<number, string>>(new Map())
  // AI 助手对话：当前活跃的 conversation_id
  const assistantConversationId = ref<string | null>(
    localStorage.getItem('assistant_conversation_id')
  )

  /** 设置指定医生的 conversation_id，同时持久化至 localStorage */
  function setDoctorConversation(doctorId: number, conversationId: string): void {
    doctorConversations.value.set(doctorId, conversationId)
    localStorage.setItem(`conversation_doctor_${doctorId}`, conversationId)
  }

  /** 获取指定医生的 conversation_id，优先从 Map 读取，降级从 localStorage 恢复 */
  function getDoctorConversation(doctorId: number): string | null {
    const fromMap = doctorConversations.value.get(doctorId)
    if (fromMap) return fromMap
    const fromStorage = localStorage.getItem(`conversation_doctor_${doctorId}`)
    if (fromStorage) {
      doctorConversations.value.set(doctorId, fromStorage)
      return fromStorage
    }
    return null
  }

  /** 设置 AI 助手的 conversation_id */
  function setAssistantConversation(conversationId: string): void {
    assistantConversationId.value = conversationId
    localStorage.setItem('assistant_conversation_id', conversationId)
  }

  /** 清除指定医生的会话（用于用户手动删除对话历史） */
  function clearDoctorConversation(doctorId: number): void {
    doctorConversations.value.delete(doctorId)
    localStorage.removeItem(`conversation_doctor_${doctorId}`)
  }
...
**Pinia riskFormStore 类型定义**：riskFormStore 用于管理风险预测多步骤表单的跨步骤数据持久化，表单数据同时写入 sessionStorage 作为备份。以下为概要设计阶段确定完整字段的参考定义：
...
export const useRiskFormStore = defineStore('riskForm', () => {
  const formData = ref<RiskFormData>(loadFromStorage())

  /** 从 sessionStorage 恢复表单数据 */
  function loadFromStorage(): RiskFormData {
    try {
      const stored = sessionStorage.getItem(STORAGE_KEY)
      if (stored) return JSON.parse(stored) as RiskFormData
    } catch { /* 忽略 JSON 解析错误 */ }
    return {
      diabetes_history: '',
      gender: '',
  ...
  /** 更新表单数据并同步写入 sessionStorage */
  function updateFormData(partial: Partial<RiskFormData>): void {
...
以上 chatStore 和 riskFormStore 的完整字段定义、方法签名和序列化策略在概要设计阶段确定。具体实现中需注意：（1）`Map` 类型在 Pinia持久化插件（如 pinia-plugin-persistedstate）中可能不被正确序列化，chatStore 采用手动 localStorage 读写策略以避免此问题；（2）riskFormStore 的 sessionStorage 持久化在页面刷新后自动恢复，用户无需重新填写已完成的步骤。

**跨浏览器标签页的登录态同步**：当用户在同一浏览器中打开多个标签页时，需处理以下场景——若用户在一个标签页中执行登出操作，其他标签页的 Pinia authStore 不会自动感知此变更，可能导致标签页 B 仍显示已登录状态而实际 API 调用返回 401。为解决此问题，系统应在应用初始化时监听 `window.addEventListener('storage', ...)` 事件：当检测到 `token` 或 `role` 键在其他标签页中被修改（设置为新值）或移除（deleteItem / clear）时，同步更新当前标签页 Pinia authStore 的状态——若 `token` 被移除则调用 `authStore.clearAuth()` 触发登出；若 `token` 被修改为其他值则同步更新 `authStore.setAuth()`。此策略利用浏览器原生的 `storage` 事件跨标签页广播 localStorage 变更，无需引入额外的跨标签页通信库。跨标签页同步的具体实现（事件监听器注册时机、防抖策略、与路由守卫的协作）在概要设计阶段确定。
...
- **用户风险信息**（user_risk_info）：用户ID、年龄、性别（gender，TEXT，CHECK(gender IN ('male', 'female'))）、身高、体重、家族史（family_history，TEXT，CHECK(family_history IN ('yes', 'no'))）、腰围、收缩压、妊娠状态、糖尿病病史（diabetes_history，TEXT，CHECK(diabetes_history IN ('healthy', 'prediabetes', 'diagnosed'))，与 POST /api/risk/predict 的 diabetes_history 参数对应，枚举值约定见 6.3 节）、原始输入信息、疾病类型（diabetes_type，TEXT，CHECK(diabetes_type IN ('type1', 'type2', 'gestational', 'other') 或 NULL)，枚举值约定见 6.3 节）。
- **生活方案**（life_plans）：方案ID、用户ID、类型（plan_type，TEXT，CHECK(plan_type IN ('diet', 'exercise', 'other'))，对应中文展示为饮食/运动/其他）、排序（order，INTEGER，控制显示顺序，与时段映射约定见4.5节）、时间、标题、内容、是否活跃（is_active，BOOLEAN，默认 TRUE，方案调整时标记旧方案为 FALSE，见 4.5 节方案调整策略）。
- **生活建议**（life_advice）：建议ID、用户ID、标题、标签、内容。该表存储 AI 智能助手在对话中为用户生成的健康生活建议内容，用户可通过 AI 助手对话历史或个人中心入口回溯查看。
- **打卡记录**（punch_in）：打卡ID、用户ID、方案项ID（plan_id，INTEGER，FOREIGN KEY REFERENCES life_plans(id)）、打卡时间、打卡类型（punch_type，TEXT，CHECK(punch_type IN ('diet', 'exercise'))，对应中文展示为饮食/运动）、完成状态（completion_status，TEXT，CHECK(completion_status IN ('completed', 'uncompleted'))，对应中文展示为已完成/未完成）、备注。plan_id 外键将每条打卡记录关联到具体的方案项，支撑按方案项维度的依从性分析。
...
**Dify 平台能力验证任务（门禁任务）**：上述双认证模式依赖 Dify 平台支持在 Text2SQL 工具回调请求体中动态引用对话上下文的 `user` 参数——即 Dify Agent 能够将 Express 代理转发时传入的 `user` 字段（当前用户的 user_id）透传至工具回调的 HTTP 请求体中。此能力并非所有 LLM Agent 平台的通用特性，且是整个 AI-to-database 授权管道的基石——若 Dify 平台不支持此能力，主方案和备选方案均无法工作。**因此本验证任务被设定为门禁任务（gate task），必须在概要设计启动前完成**，不得推迟至开发环境搭建阶段。验证方法：在 Dify 平台上创建一个测试 Agent，挂载一个指向 Express 测试端点的 HTTP 工具，在工具回调请求体模板中引用 `{{user}}` 变量，通过 Express 代理发送测试消息并观察回调请求体中是否实际携带了 user_id。

**依赖此能力的核心功能模块**（以下模块的技术方案以 Dify `{{user}}` 变量透传能力为前提，在验证完成前标注为"待 Dify 平台能力验证后确认"）：
- 4.8 AI 智能助手 — Text2SQL 操作（普通用户通过 Agent 查询/修改本人数据）
- 4.9 智能管理 — 管理员自然语言数据库操作（管理员通过 Agent 操作全库数据）
- 5 节 AI 驱动的 Text2SQL 路径 — 整体数据操作路径
- 5 节 Dify Agent 回调 `/api/admin/execute` 的用户身份传递机制 — 双认证模式

**验证未通过时的替代架构方案**：若 Dify 平台不支持 `{{user}}` 变量透传，采用以下方案——Express 代理层在转发用户请求至 Dify Agent 前，根据 JWT Token 中的 user_id 预查询该用户在数据库中的相关信息（如个人信息、风险记录、打卡数据等），将查询结果预先注入 Dify Agent 的对话上下文（通过 Dify API 的 `inputs` 参数传入），使 Agent 在回答用户问题时无需通过 Text2SQL 工具回调 Express 即可获取所需用户数据。此方案下 Agent 不再需要携带 user_id 进行回调，但代价是：（1）Agent 的写操作（如修改用户数据、写入打卡记录）需改为在前端通过常规 CRUD 端点完成，Agent 仅提供决策建议文本；（2）管理员的自然语言数据库操作需改为在 Express 代理层预查询后注入上下文，管理员写操作同样退回常规 CRUD 端点。此替代方案的完整设计留待门禁验证结果确定后补充。备选方案需注意以下关键实现约束：（1）**session_id 传递仍依赖 `{{user}}` 变量透传**——Express 代理请求中通过 Dify 的 `user` 参数传入 session_id，Dify Agent 仍需通过 `{{user}}` 变量将 session_id 填入工具回调请求体。若 Dify 平台不支持 `{{user}}` 变量透传，主方案和备选方案均无法工作，此时需进一步退化为方案 B——由管理员在智能管理界面中手动输入 user_id 或在 Agent 系统提示词中写死测试用户 ID（仅适用开发和演示阶段，不适用生产）。此约束应在 Dify 平台能力验证任务中同步验证；（2）**映射表存储位置**——推荐使用 Express 进程内存 Map（无需外部依赖，与 SQLite 单实例部署一致），会话状态在 Express 重启后丢失（用户需重新登录，属可接受的行为）。若项目期望会话状态持久化，可在 SQLite 数据库中创建 `dify_session_map` 表（session_id TEXT PRIMARY KEY、user_id TEXT、created_at TIMESTAMP）替代内存 Map；（3）**映射表生命周期**——条目在用户主动登出时清理（调用 authStore.clearAuth() 时前端请求 Express 清理端点），同时在 JWT Token 过期时清理（Express 定时任务扫描过期 Token 对应的映射条目，清理周期与 JWT_EXPIRES_IN 一致）。备选方案的完整设计（含清理策略、并发安全性、session_id 生成算法）留待概要设计阶段补充（待主方案验证结果确定是否需要启用）。
...
| JWT Token 中的 `user_id` | Dify 工作流 `inputs.user_id` / Dify Agent `user` 参数 | Express 从 JWT Token 中提取 user_id 后自动填充至 Dify API 请求的 `user` 参数（用于 Dify 平台的用户会话隔离）。对于需要用户健康数据的工作流（如 health-article-generator、punch-analysis），若工作流 `inputs` 中包含 `user_id` 字段，Express 同步将 user_id 填入 `inputs.user_id` 以便工作流内部查询数据库。此映射适用于所有专用端点（POST /api/articles/generate、POST /api/plan/generate、GET /api/punch/analysis 等）和通用代理端点 |
...
端点 `/api/auth/login` 正常响应（HTTP 200）：
...
```json
{
  ...
  "user": {
    "id": 1,
    "username": "zhangsan",
    "avatar": "/static/uploads/avatars/1_1719123456.jpg"
  }
}
```

- `token`（string）：JWT Token，前端存储于 localStorage 并在后续 API 请求的 Authorization 头中携带（格式 `Bearer <token>`）
- `role`（string）：用户角色，取值 `user` 或 `admin`，前端用于控制界面元素显隐和路由守卫逻辑
- `user`（object）：用户基本信息，含 id、username 和 avatar（可选），用于前端展示当前登录用户头像和用户名

端点 `/api/auth/register` 正常响应（HTTP 201）：与登录响应结构一致（注册成功后直接返回 JWT Token 和用户信息，用户无需重复登录）。以上响应结构标注为基于 JWT 认证通用实践的推断，具体字段名和嵌套层级在概要设计阶段确定。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| diabetes_history | string | 是 | 用户的糖尿病病史状态。枚举值：`"healthy"`（健康）、`"prediabetes"`（糖尿病前期）、`"diagnosed"`（已确诊）。其他值将被服务端拒绝 |
| diabetes_type | string | 否 | 若 diabetes_history 为 `"diagnosed"`，需填写糖尿病类型。枚举值：`"type1"`（1型）、`"type2"`（2型）、`"gestational"`（妊娠期）、`"other"`（其他特殊类型）。其他值将被服务端拒绝 |
| age | number | 是 | 用户年龄 |
| gender | string | 是 | 用户性别。枚举值：`"male"`（男）、`"female"`（女）。其他值将被服务端拒绝 |
...
| family_history | string | 是 | 家族糖尿病史。枚举值：`"yes"`（有）、`"no"`（无）。其他值将被服务端拒绝 |
| pregnancy | boolean | 否 | 是否妊娠，仅 gender 为 `"female"` 时有效 |
...
端点 `/api/risk/predict` 正常响应（HTTP 200）：
...
```json
{
  ...
  "advice": "根据您的健康数据，您的糖尿病风险评分为 32 分（满分 51 分），属于高风险人群。最可能匹配的类型为 2 型糖尿病……",
  "record_id": 42
}
```

- `risk_score`（number）：风险评分，0-51 分区间，≥25 分为高风险
- `risk_level`（string）：风险等级枚举值——`low`（低风险，<15 分）、`medium`（中风险，15-24 分）、`high`（高风险，≥25 分）
- `risk_level_label`（string）：风险等级中文标签，前端用于直接展示
- `matched_diabetes_type`（string）：若为高风险，输出最可能匹配的糖尿病类型名称（1型糖尿病/2型糖尿病/妊娠期糖尿病/其他特殊类型）；若为中低风险，此字段可省略或为 null
- `advice`（string）：个性化的预防或管理建议文本（Markdown 格式，前端渲染为富文本）
- `record_id`（number）：写入 user_risk_info 表后返回的记录 ID，用于后续历史查询和引用

GET /api/risk/history 正常响应（HTTP 200）——采用 6.12 节分页响应外壳，`data` 数组元素结构为上述响应字段的子集（含 risk_score、risk_level、risk_level_label、created_at 等），完整字段定义在概要设计阶段确定。以上响应结构标注为基于风险预测功能的逻辑推断，具体字段名和风险等级枚举值在概要设计阶段根据 Dify 工作流输出格式确定。
...
GET /api/doctors 正常响应（HTTP 200）——采用 6.12 节分页响应外壳，`data` 数组元素结构：
...
```json
{
  ...
  "department": "内分泌科",
  "title": "主任医师",
  "description": "擅长糖尿病综合管理...",
  "avatar": "/static/images/doctors/zhang.jpg"
}
```

各字段对应 doctor_information 表中的列（id、name、department、title、description、avatar）。注意：chat_token 字段不在响应中返回——该字段仅在服务端 Express 代理层使用，不暴露给前端。GET /api/doctors/:id 的响应结构与上述元素结构相同，但不套用分页外壳，直接返回单个医生对象的完整字段。以上响应结构为基于 doctor_information 表结构的推断，具体字段名在概要设计阶段确定。
...
SSE 代理转发策略：Express 服务端根据 `:id` 从 doctor_information 表读取 chat_token，以该 Token 向 Dify 聊天助手 API 发起 SSE 流式请求。Express 以流模式（Transfer-Encoding: chunked）将 Dify 返回的 SSE 事件逐块透传至前端，不缓冲完整响应后再发送。每个 SSE 事件保持 Dify 原始格式（`data: {...}\n\n`），前端使用 fetch + ReadableStream 方式消费流数据。因认证 SSE 端点需在请求头中携带 JWT Token（Authorization: Bearer \<token\>），而浏览器标准的 EventSource API 不支持自定义 HTTP 请求头，故仅推荐使用 fetch API 实现流式消费。chat_token 始终保持服务端侧，不暴露给前端。SSE 事件 data 字段的详细结构说明见 6.9 节"SSE 事件 data 字段结构说明"——该结构适用于所有 SSE 流式端点（医师对话、AI 助手、管理员对话）。
...
POST /api/plan/generate 的服务端处理流程：接收前端提交的用户信息和偏好 → Express 从 JWT Token 中提取 user_id → 将 user_id 作为 Dify API 请求的 `user` 参数传入并同步填入工作流 `inputs.user_id`（供工作流内部查询用户健康数据使用）→ 调用 Dify life-plan-generator 工作流 → 接收工作流返回的饮食和运动方案内容 → 将方案项逐条解析并写入 life_plans 表（含类型、排序、时间、标题、内容）→ 返回方案列表给前端。方案项的排序(order)值由工作流返回或由 Express 按 4.5 节约定的时段映射自动分配。PUT /api/plan/adjust 的处理流程类似——接收修改意见后重新调用工作流，新生成的方案替换原有方案记录（通过 is_active 逻辑过期机制，见 4.5 节方案调整策略）。

端点 `/api/plan/generate` 正常响应（HTTP 200）：
...
```json
{
  ...
  "items": [
    ...
    },
    {
      ...
      "time": "12:00",
      "title": "午餐",
      "content": "杂粮饭（100g）+ 清蒸鱼（150g）+ 蒜蓉西兰花..."
    }
  ]
}
```
...
- `items`（array）：方案项数组，每个元素含 id（number，life_plans 表主键）、type（string，饮食/运动）、order（number，排序，映射约定见 4.5 节）、time（string，时间节点，如"07:30"或"晨间"）、title（string，方案项标题）、content（string，具体内容，Markdown 格式）

GET /api/plan/current 正常响应：结构与上述 POST 响应一致（返回当前活跃方案的所有方案项）。PUT /api/plan/adjust 正常响应：结构与 POST /api/plan/generate 一致（调整即重新生成新方案）。以上响应结构标注为基于 life_plans 表结构和 4.5 节时段映射约定的推断，具体字段名和嵌套层级在概要设计阶段确定。
...
| punch_type | string | 是 | 打卡类型。枚举值：`"diet"`（饮食）、`"exercise"`（运动）。其他值将被服务端拒绝 |
| completion_status | string | 是 | 完成状态。枚举值：`"completed"`（已完成）、`"uncompleted"`（未完成）。其他值将被服务端拒绝 |
...
POST /api/articles/generate 的服务端处理流程：接收前端提交的分类标签选择 → Express 从 JWT Token 中提取 user_id → 将 user_id 作为 Dify 工作流 inputs 中的 `user_id` 参数传入 → 调用 Dify health-article-generator 工作流 → 接收工作流返回的文章标题、标签列表、Markdown 正文、封面 URL → 将文章结构化写入 articles 表 → 返回文章详情给前端。若 Dify 工作流在分类生成阶段（未传 category 参数），Express 先将返回的分类标签列表返回前端供用户选择，用户选择后再触发文章生成阶段。两阶段的具体实现方式在概要设计阶段确定。

GET /api/articles 正常响应（HTTP 200）——采用 6.12 节分页响应外壳，`data` 数组元素结构（列表摘要，不含完整正文）：

```json
{
  "id": 1,
  "title": "糖尿病患者日常饮食指南",
  "cover": "https://example.com/images/article_1.jpg",
  "author": "AI 健康助手",
  "publish_time": "2026-06-23T10:00:00Z",
  "category": "饮食指导",
  "tags": ["糖尿病", "饮食", "血糖控制"],
  "read_count": 1280,
  "summary": "本文从碳水化合物控制、蛋白质摄入..."
}
```

GET /api/articles/:id 正常响应（HTTP 200）——包含完整正文的单篇文章详情：
...
- `content`（string）：Markdown 格式的文章正文，前端使用 marked 解析器渲染为 HTML
- `is_collected`（boolean）：当前登录用户是否已收藏此文（未登录时为 false）。此字段为推断补充，用于前端在文章详情页直接渲染收藏按钮的已收藏/未收藏状态，避免额外的收藏状态查询请求

POST /api/articles/generate 正常响应：结构与上述文章详情一致（生成后直接返回完整文章）。以上响应结构标注为基于 articles 表结构的推断，具体字段名和嵌套层级在概要设计阶段确定。
...
**SSE 事件 data 字段结构说明**（适用于所有 SSE 流式端点——POST /api/chat/doctor/:id、POST /api/assistant/chat、POST /api/admin/chat）：Dify 对话 API 的 SSE 事件遵循 Dify 标准流式响应格式，每个 SSE 事件的 `data` 字段为 JSON 字符串，核心字段如下（以 Dify 对话 API 官方格式为参考，实际格式以 Dify 平台运行版本为准）：
...
- `event`（string）：事件类型——`message`（正常消息块，answer 字段含增量文本）、`message_end`（消息结束，answer 为空，前端确认流结束）、`error`（流内逻辑错误，message 字段含错误描述，处理策略见 6.4 节 SSE 流内错误事件处理）、`workflow_started`/`workflow_finished`（工作流执行开始/结束，仅工作流类型端点）、`agent_thought`（Agent 推理过程，仅 Agent 类型端点）
- `conversation_id`（string）：Dify 返回的会话 ID，前端应在首次收到有效 conversation_id 后保存至 Pinia store 和 localStorage（对应医生 ID 或 AI 助手）
- `message_id`（string）：消息 ID，用于前端在对话界面的消息去重和引用
- `answer`（string）：增量回答文本（Markdown 格式），前端在 ReadableStream 消费循环中逐块拼接渲染。注意：一个完整的回答可能由多个 `event: "message"` 的 SSE 事件组成，前端需累积拼接而非每个事件独立渲染为独立气泡
- `created_at`（number）：Unix 时间戳（秒），消息创建时间

以上 SSE 事件结构标注为基于 Dify API 通用规范的推断。Dify 不同版本（社区版/云版）的 SSE 事件格式可能存在差异——具体字段名、事件类型枚举值和附加字段（如 `metadata`、`usage` 等）以 Dify 平台部署版本的实际输出为准，开发环境搭建阶段需实测确认。
...
SSE 代理转发策略：Express 服务端以流模式透传 Dify admin-manager-agent 返回的 SSE 事件至前端，管理员在对话界面中查看自然语言转 SQL 的执行过程和结果。前端使用 fetch + ReadableStream 方式消费流数据（不使用 EventSource API，原因同 6.4 节：认证 SSE 端点需在请求头中携带 JWT Token，而 EventSource 不支持自定义请求头）。SSE 流内错误事件处理策略与 6.4 节一致（按 `event` 字段区分错误类型并分类处理）。SSE 事件 data 字段结构说明见 6.9 节——该结构适用于所有 SSE 流式端点。
...
**定位说明**：以下通用 Dify 代理端点为 Express 内部代理层的通用转发端点，由各专用端点（如 6.3-6.10 节定义的 POST /api/risk/predict、POST /api/plan/generate、POST /api/articles/generate、POST /api/chat/doctor/:id、POST /api/assistant/chat、POST /api/admin/chat 等）在服务端内部调用，**不直接暴露给前端**。架构分层为：**前端 → 专用 API 端点（业务语义层）→ 通用 Dify 代理端点（转发层）→ Dify API**。通用代理端点的存在使 Express 代理层的 Dify 请求转发逻辑集中复用，避免各专用端点中重复实现 Dify API 调用、参数映射和错误处理逻辑。

| 方法 | 端点 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/dify/workflow/:workflow_id` | 代理转发至 Dify 工作流 API，触发工作流执行。由各专用端点（如 POST /api/risk/predict 等）在服务端内部调用，不直接暴露给前端 | 是（服务端内部携带 JWT 派生的 user_id） |
| POST | `/api/dify/agent/:agent_id` | 代理转发至 Dify Agent API，触发 Agent 对话。由专用端点 POST /api/assistant/chat 和 POST /api/admin/chat 在服务端内部调用，不直接暴露给前端 | 是（服务端内部携带 JWT 派生的 user_id） |
...
|----------------------|------|------|------------|---------|-----------------|---------------------|
| `diabetes-risk-prediction` | 工作流 | 根据用户健康数据计算糖尿病风险评分并生成建议 | 4.4 糖尿病风险预测 | POST /api/risk/predict（专用端点）→ 内部调用 `/api/dify/workflow/:workflow_id` | `age` (number, 必填)、`gender` (string, 必填)、`height` (number, 必填)、`weight` (number, 必填)、`waist` (number, 可选，已在4.4节定义估算公式)、`systolic_bp` (number, 可选，已在4.4节定义估算公式)、`family_history` (string, 必填)、`pregnancy` (boolean, 仅女性，已在4.4节定义)、`diabetes_history` (string, 必填) | 输出写入 user_risk_info 表（见 6.3 节处理流程） |
| `life-plan-generator` | 工作流 | 根据用户身体信息和生活习惯生成个性化饮食与运动方案 | 4.5 生活方案 | POST /api/plan/generate（专用端点）→ 内部调用 `/api/dify/workflow/:workflow_id` | `health_info` (object, 必填，含age/gender/height/weight等已在4.5节定义)、`preferences` (object, 必填，含dietary/activity偏好，已在4.5节定义) | 输出写入 life_plans 表（见 6.5 节处理流程） |
| `health-article-generator` | 工作流 | 根据用户健康信息生成健康资讯分类标签和完整文章 | 4.6 健康资讯 | POST /api/articles/generate（专用端点）→ 内部调用 `/api/dify/workflow/:workflow_id` | `user_id` (string, 必填，用于查询用户健康数据，数据来源优先级已在4.6节定义)、`category` (string, 可选，用户选择的分类标签) | 输出写入 articles 表（见 6.7 节处理流程） |
| `punch-analysis` | 工作流 | 对用户打卡记录进行汇总分析和趋势解读 | 4.7 打卡记录与分析 | GET /api/punch/analysis（专用端点）→ 内部调用 `/api/dify/workflow/:workflow_id` | `user_id` (string, 必填)、`date_range` (object, 可选，含start/end日期，已在4.7节定义筛选支持) | 不持久化（分析结果即时返回，不写入数据库） |
| `diabetes-assistant-agent` | Agent | 通过工具调用完成知识问答、风险预测、方案生成、信息查询等综合操作 | 4.8 AI 智能助手 | POST /api/assistant/chat（专用端点）→ 内部调用 `/api/dify/agent/:agent_id` | `query` (string, 必填，用户的自然语言消息，已在6.9节定义) | 健康建议输出写入 life_advice 表（由 Agent 工具调用触发） |
| `admin-manager-agent` | Agent | 将管理员的自然语言指令转为SQL操作数据库 | 4.9 智能管理 | POST /api/admin/chat（专用端点）→ 内部调用 `/api/dify/agent/:agent_id` | `query` (string, 必填，管理员的自然语言指令，已在6.10节定义) | 操作结果写入 admin_logs 表（操作日志） |
| `doctor-chat-{id}` | 聊天助手 | 以特定科室医生的角色进行一对一健康咨询对话 | 4.2 医师咨询 | POST /api/chat/doctor/:id（专用端点）→ 内部调用 Dify 聊天助手 API | `query` (string, 必填，用户的对话消息，已在6.4节定义) | 不持久化（对话内容即时返回，不写入数据库） |
...
- Agent 类型：通过 Dify Agent API 触发，Agent 挂载的工具链在 Dify 平台上配置。以下为两个 Agent 的预期工具清单（基于功能需求和 Agent 通用设计模式的推断，完整工具定义和具体参数 schema 在概要设计阶段确定）。

**diabetes-assistant-agent 预期工具清单**（AI 智能助手，服务于 4.8 节）：

| 工具名称（推荐命名） | 用途 | 输入类型 | 输出类型 | 权限约束 | 前端交互影响 |
|-------------------|------|---------|---------|---------|------------|
| `query_user_profile` | 查询当前用户个人信息 | user_id（由 Agent 从对话上下文获取） | 用户基本信息 JSON | 仅限查询当前 user_id 的本人数据 | 仅后端执行，前端无需特殊渲染 |
| `query_risk_history` | 查询用户历史风险预测记录 | user_id | 风险预测记录列表 JSON | 仅限查询当前 user_id 的本人数据 | 仅后端执行，前端无需特殊渲染 |
| `query_punch_records` | 查询用户打卡记录（支持日期范围和类型筛选） | user_id、startDate（可选）、endDate（可选）、punch_type（可选） | 打卡记录列表 JSON | 仅限查询当前 user_id 的本人数据 | 仅后端执行，前端无需特殊渲染 |
| `query_life_plans` | 查询用户当前活跃方案 | user_id | 方案项列表 JSON | 仅限查询当前 user_id 的本人数据 | 仅后端执行，前端无需特殊渲染 |
| `query_health_advice` | 查询用户历史健康建议 | user_id | 健康建议列表 JSON | 仅限查询当前 user_id 的本人数据 | 仅后端执行，前端无需特殊渲染 |
| `write_health_advice` | 将健康生活建议写入 life_advice 表 | user_id、title、tags、content | 写入结果确认 | 仅限写入当前 user_id 的本人数据；需 INSERT 权限 | 仅后端执行，Agent 在对话中告知用户"已生成健康建议，可在我的-健康建议中查看" |
| `update_user_profile` | 修改用户个人信息（如用户名、健康数据补充） | user_id、字段名、新值 | 更新结果确认 | 仅限修改当前 user_id 的本人数据；需 UPDATE 权限 | 前端需展示"操作执行成功/失败"反馈气泡；涉及敏感字段修改时建议二次确认 |
| `knowledge_search` | 检索糖尿病相关知识库（基于 Dify 知识库） | 自然语言查询 | 相关知识片段 | 无行级限制（知识库为只读共享资源） | 仅后端执行，检索结果内嵌在 Agent 回复中 |

**admin-manager-agent 预期工具清单**（智能管理，服务于 4.9 节）：

| 工具名称（推荐命名） | 用途 | 输入类型 | 输出类型 | 权限约束 | 前端交互影响 |
|-------------------|------|---------|---------|---------|------------|
| `query_table` | 查询任意数据表内容（支持条件筛选和分页） | 表名、WHERE 条件（可选）、ORDER BY（可选）、LIMIT/OFFSET（可选） | 查询结果 JSON | role=admin，可查询所有表 | 仅后端执行，查询结果在对话界面中展示为结构化表格或列表 |
| `insert_record` | 向指定数据表插入新记录 | 表名、字段键值对 | 插入结果确认（含新记录 ID） | role=admin，可向所有表插入；需 INSERT 权限 | 前端展示"操作成功/失败"反馈气泡；涉及敏感表（如 users）建议展示操作预览后二次确认 |
| `update_record` | 更新指定数据表中的已有记录 | 表名、WHERE 条件、字段键值对 | 更新结果确认（含影响行数） | role=admin，可更新所有表；需 UPDATE 权限 | 前端展示"操作成功/失败"反馈气泡；操作前展示影响行数预览，建议二次确认 |
| `delete_record` | 删除指定数据表中的记录 | 表名、WHERE 条件 | 删除结果确认（含影响行数） | role=admin，可删除所有表；需 DELETE 权限 | 前端必须展示二次确认弹窗（含影响行数预览），用户确认后方可执行；删除操作不可撤销 |
| `get_table_schema` | 获取数据表结构信息（列名、类型、约束） | 表名 | 表结构 JSON | role=admin，可查看所有表结构 | 仅后端执行，表结构信息在对话界面中展示为技术性信息 |

以上工具清单的完整定义（输入输出 JSON Schema、错误码、Dify 平台上的工具配置参数）在概要设计阶段确定。各工具的"前端交互影响"列标注了前端在 AI 助手/智能管理对话界面中需实现的特殊交互行为——如二次确认弹窗、操作反馈气泡、结构化结果渲染等，前端开发者在概要设计阶段应据此设计对应的对话消息渲染组件。
...
以上设计参数为基于移动端医疗健康类应用通用设计模式的推断，具体色值、字号和间距在 UI 设计阶段可根据视觉设计方案调整，但需保持各模块间的一致性。若采用 Vant 4 作为移动端 UI 组件库（见 8.1 节），设计系统的 CSS 变量应与 Vant 4 的主题变量体系（如 `--van-primary-color`、`--van-font-size-md` 等）建立映射关系，确保自定义样式与组件库主题统一。具体 CSS 变量映射表在概要设计阶段确定。
...
- 涉及 AI 内容生成的操作（方案生成、文章生成、风险分析、AI 对话）允许更长的等待时间，但应提供加载状态提示，单次 AI 请求应在 15 秒内开始返回内容（基于大模型 API 调用的一般经验推断，此数字为合理预期而非硬性约束）。各功能模块验收标准中的时间指标（3 秒/10 秒/15 秒/1 秒）统一以用户触发操作的时刻为计时起点（如点击按钮、发送消息），而非服务端收到请求的时刻。前端计时与服务端计时可能存在差异——前端计时包含网络传输耗时，服务端计时仅计算 Dify/DeepSeek 处理耗时。各验收标准中以"合理预期而非硬性约束"标注的指标，其实际值可能受 DeepSeek API 负载和网络条件影响。
- 数据同步操作（如打卡记录写入）在 1 秒内完成。
- 系统在设计运行时段内保持可用，计划内维护窗口除外。当服务器1发生宕机时，服务恢复时间目标（RTO）为人工介入恢复时长（通常 30 分钟内）；数据恢复依赖定期备份策略（见 7.4 节 SQLite 数据可靠性说明）。作为实训项目，系统不要求商业级高可用（如自动故障转移或热备切换），可用性目标以保障开发调试和演示评估期间的正常运行为准。
...
- **可移植性**：平台基于 Vue3 + Express（TypeScript 全栈 Web 技术）构建，天然支持跨操作系统运行——Vite 构建产物（含 HTML/JS/CSS 静态资源）可在不同 Windows、Linux、macOS 服务器环境之间迁移部署，无需修改源代码。
...
| 服务器 | 角色 | 部署组件 | 说明 |
|--------|------|---------|------|
| 服务器1 | 数据服务器 | SQLite、Express（端口3000）、Nginx（静态文件服务） | Express 提供 REST API 接口层，SQLite 存储所有业务数据，Nginx 托管前端 Vite 构建产物（含 HTML/JS/CSS 静态资源）。此服务器为系统的数据和逻辑中心 |
...
**前端部署**：Vite 构建产物（含 HTML/JS/CSS 静态资源）部署在服务器1的 Nginx 静态文件目录下。用户浏览器直接访问 Nginx 获取前端资源。
...
**静态资源托管**：Nginx 除托管前端 Vite 构建产物（含 HTML/JS/CSS 静态资源）外，还需配置以下静态资源目录的访问路径：
...
**SQLite 数据可靠性说明**：SQLite 为文件型数据库，数据文件仅存在于服务器1的本地磁盘上，不具备主从复制或自动故障转移能力。当服务器1发生宕机、硬件故障或磁盘损坏时，API 服务和数据访问将全部中断，服务器2和服务器3的 Nginx 仅能提供静态资源负载均衡，无法接管数据服务。因此，虽然部署架构使用3台服务器，但在数据可用性层面实际为单点架构——此约束已体现在 7.2 节性能要求的修订后的可用性表述中（不承诺 7x24 连续运行，RTO 为人工介入恢复时长）。项目应建立数据库文件的定期备份策略（如每日定时备份 SQLite 数据库文件至服务器2或外部存储），以降低数据丢失风险。具体备份方案在部署设计阶段确定。
...
- 移动端优先的 Web 应用（**Vue 3 + TypeScript** + **Vite** 构建工具实现）
- 前端构建方式：采用 **Vue 3 + TypeScript + Vite** 技术栈开发。Vue 3 提供组件化开发（SFC .vue 单文件组件）、Composition API、响应式状态管理（Pinia）、声明式路由（Vue Router）；TypeScript 提供静态类型检查、接口定义和编译期错误拦截；Vite 提供开发服务器（HMR 热更新）和生产构建（代码分割、Tree Shaking、资源压缩）。第三方库通过 npm 安装管理，在 `vite.config.ts` 中配置按需引入。**移动端 UI 组件库选型**：推荐采用 **Vant 4**（移动端优先的 Vue 3 组件库），提供 Tabbar、ActionSheet、DatetimePicker、PullRefresh、Toast、Dialog 等移动端特有交互组件，避免从零开发。Vant 4 支持按需引入（Tree Shaking），Bundle 体积可控，且内置主题定制能力（CSS Variables），可与 7.1 节设计系统参数对接。若项目组有意推迟选型至概要设计阶段，需在此阶段至少评估以下维度后确定：（1）移动端适配完整性（Tabbar/ActionSheet/PullRefresh/DatetimePicker 等）；（2）Vue 3 + TypeScript 兼容性；（3）Bundle 体积与按需引入支持；（4）社区活跃度与文档完善程度。Swiper 轮播组件和 marked Markdown 解析器作为独立 npm 依赖单独引入（前者用于首页 Banner 轮播，后者用于文章正文 Markdown 渲染）。**Cline + DeepSeek** 作为 AI 编程助手辅助前后端代码生成——Cline 调用 DeepSeek 大模型生成 Vue 3 + TypeScript 组件代码、API 调用代码、Express 路由处理逻辑等。此选择基于以下推断：（1）Vue 3 是渐进式框架，学习曲线平缓，适合实训项目的开发周期；（2）TypeScript 增强代码可靠性和可维护性；（3）Vite 提供极快的冷启动和 HMR，提升开发效率；（4）组件化架构消除 iframe 引入的复杂性和运行时开销；（5）Cline + DeepSeek 组合是本次实训指定的 AI 编程工具链。
- 前端源代码推荐目录结构（Vue 3 + Vite + TypeScript 标准项目结构）：`src/views/`（页面级 Vue SFC 组件，.vue 文件）、`src/components/`（可复用 Vue 组件，.vue 文件）、`src/router/`（Vue Router 路由配置，index.ts）、`src/stores/`（Pinia store 模块，.ts 文件）、`src/api/`（API 请求封装模块，含 Axios 拦截器，.ts 文件）、`src/types/`（TypeScript 类型定义与接口声明，.d.ts 文件）、`src/composables/`（Vue 3 组合式函数，.ts 文件）、`src/assets/`（静态资源——图片、全局 CSS/SCSS）、`src/utils/`（工具函数，.ts 文件）、`public/`（不经过构建的静态文件）、`index.html`（Vite 入口 HTML）、`vite.config.ts`（Vite 构建配置）、`tsconfig.json`（TypeScript 编译配置）。具体目录命名和文件划分在概要设计阶段确定。
...
1. **导航结构**：原始需求仅列出功能模块，未说明模块间的组织方式。本文将其明确为 Vue 3 SPA 架构下的底部 Tab 栏 + Vue Router 路由导航方式，所有模块运行在同一 Vue 运行时中（v2 修订：此前 v1 版本基于项目任务文档中"底部 Tab 栏 + iframe 子页面"的描述做推断，现已统一为 Vue Router SPA 架构）。
...
11. **iframe 跨模块通信**（v2 修订：已在 v1→v2 修订中删除）：原始需求和项目任务文档均未涉及 iframe 架构下的跨模块通信问题。本文 v1 版本在 1.1 节补充了登录态共享、FAB 渲染层级、AI 助手触发导航、iframe 间数据传递四项通信机制的设计原则（v3 新增，标注为基于架构特性的推断）。**v1→v2 修订中已删除第 1.1 节全部 iframe 通信机制描述，统一为 Vue Router SPA 架构下的 Pinia 状态管理。**

12. **用户认证与会话管理**：原始需求未涉及认证机制的具体实现方式。本文新增 4.10 节，明确 JWT 无状态会话机制、Token 的生成/存储/传递/校验流程、前后端鉴权方式、Pinia authStore 统一登录态管理、bcrypt 密码哈希存储等完整认证体系（v3 新增，标注为基于 Web 安全通用实践的推断；v2 修订：将"iframe 间登录态共享"更新为"Pinia authStore 统一登录态管理"以匹配 Vue 3 SPA 架构）。
...
以下修订说明章节记录了本文档从初始版本至今的完整演进过程。**各节按时间线从旧到新排列如下**，建议按此顺序依次阅读以理解文档的逐步完善轨迹：

| 阅读顺序 | 章节 | 修订批次 | 说明 |
|---------|------|---------|------|
| 1 | 第 17 节 | v1 → v2 | **最早修订批次**：统一前端技术栈为 Vue 3 + TypeScript + Vite，消除 iframe 与 SPA 二义性 |
| 2 | 第 10 节 | v2 → v3 | 补充数据模型字段、认证机制、API 接口规格等核心缺口 |
| 3 | 第 11 节 | v3 → v3_rev1 | 修复密码处理矛盾、数据路径歧义等阻塞性问题 |
| 4 | 第 12 节 | v3_rev1 → v4 | 补充初始数据种子、糖尿病类型 API、Dify 工作流清单等 |
| 5 | 第 13 节 | v4 → v5 | 权限模型修正、图片存储策略、AI 持久化路径、环境配置等 |
| 6 | 第 14 节 | v5 → v5_rev2 | 数据路径前导句修复、Dify Agent 回调双认证模式 |
| 7 | 第 15 节 | v5_rev2 → v6 | 请求体参数补充、SSE 规范修正、会话管理澄清等 |
| 8 | 第 16 节 | v6 → v7 | 管理员对话端点、life_advice 写入路径、SPA 路由结构、交互状态规范等 |
| 9 | 第 18 节 | v7 → v8 | API 响应数据结构补充、时间指标计时起点、SSE 连接管理、方案调整策略等 |
...
**修订**：
- 在第 4.4 节新增"表单数据跨步骤持久化"段落，推荐使用 sessionStorage 存储表单数据的跨步骤持久化方案（每步完成后序列化写入、每步进入时回填、提交成功或重填时清除）。
- 补充说明此机制依赖所有 iframe 页面同源部署的前提条件（v2 修订：此前提条件描述已在 v1→v2 修订中随 iframe 架构移除而被替换，当前 4.4 节表单持久化采用 Pinia store + sessionStorage 方案，不再依赖 iframe 同源前提）。
...
**修订**：
- 在第 8.1 节"在范围内"列表中新增两条：（1）前端构建方式说明——采用原生 HTML/CSS/JavaScript 开发，不依赖构建工具，第三方库通过 CDN 或本地 lib 目录引入，并说明了选择的推断依据；（2）前端源代码推荐目录结构——按 pages/css/js/lib/images/uploads 分层组织。（v2 修订：此条记录描述的内容已在 v1→v2 修订中被替换，当前 8.1 节已统一为 Vue 3 + TypeScript + Vite 技术栈，目录结构已更新为 Vue 3 + Vite + TypeScript 标准结构。）
...
> **【阅读提示】** 本节为全部修订批次中**时间线最早**的一批（v1→v2），记录了前端技术栈统一、iframe 架构移除等奠基性修订。按时间线阅读时应**优先阅读本节**，再依次阅读第 10-16 节、第 18 节。详见上方"修订说明阅读指引"表格。

本轮修订基于 v1 版需求澄清文档的审查报告（`a_v2_iteration_requirement.md`，即组件B诊断报告 b_v3_diag_v2.md 经质询确认结果）中提出的 10 个质量问题，结合前端技术栈统一修订指令，逐一回应如下。10 个问题中有 2 个为阻塞性问题（必解）、3 个为高严重度问题（强建议）、3 个为中等严重度问题（建议本轮一併处理）、2 个为低严重度问题。

### 17.1 第 1 节前端技术栈声明修订（审查问题-阻塞性，问题1）

**审查意见**：第 1 节第 1 段仍保留"平台以 HTML、CSS、JavaScript 为前端技术栈"，未按修订指令修改。

**修订**：
- 将第 1 节第 1 段"平台以 HTML、CSS、JavaScript 为前端技术栈"修改为"平台以 Vue 3 + TypeScript + Vite 为前端技术栈"。
- 同步将"后端借助 DeepSeek 大模型"技术栈描述中的前端部分保持一致。

### 17.2 前端架构二义性消除——选定 Vue Router SPA（审查问题-阻塞性，问题2）

**审查意见**：文档同时描述两套互斥的前端架构——（A）iframe 子页面模式 + postMessage 跨模块通信协议（第 1.1 节）；（B）Vue Router SPA 架构 + 路由映射表（第 1.2 节）。两套架构无法共存。

**修订**（选择 Vue Router SPA 架构）：
- 删除原第 1.1 节全部内容（iframe 架构下的跨模块通信机制，含 postMessage 消息协议规范、三种消息类型定义表、消息监听器生命周期说明）。
- 将原第 1.2 节"前端路由结构"重编号为第 1.1 节。
- 将第 1 节第 2 段"各模块以 iframe 方式嵌入主框架"修改为"所有模块运行在同一 Vue 运行时中，不使用 iframe 架构"。
- 删除第 4.10 节"Vue3 SPA 的登录态共享"段落末尾的"具体实现方式见 1.1 节"交叉引用，替换为完整的 Pinia authStore 定义示例（TypeScript）。
- 修正第 4.4 节风险预测表单持久化描述，从 iframe 同源前提改为 Pinia store + sessionStorage 方案（已在 v6→v7 的 16.12 修订记录中追加说明）。
- 修正第 9 节澄清说明第 1 条：将"基于项目任务文档中'底部 Tab 栏 + iframe 子页面'架构"更新为 Vue 3 SPA 架构描述，并标注此为 v2 修订。
- 修正第 9 节澄清说明第 11 条：补充 v2 修订声明——iframe 通信机制已在 v1→v2 修订中删除。
- 修正第 9 节澄清说明第 12 条：将"iframe 间登录态共享"更新为"Pinia authStore 统一登录态管理"。

### 17.3 第 16.14 节修订记录遗留矛盾修复（审查问题-高，问题3）

**审查意见**：v6→v7 修订记录记载"采用原生 HTML/CSS/JavaScript 开发"，与当前第 8.1 节正文的 Vue 3 + Vite 描述矛盾。

**修订**：
- 在第 16.14 节修订记录末尾追加说明：该条记录描述的内容已在 v1→v2 修订中被替换，当前 8.1 节已统一为 Vue 3 + TypeScript + Vite 技术栈，目录结构已更新为 Vue 3 + Vite + TypeScript 标准结构。

### 17.4 第 16.12 节修订记录残留 iframe 架构前提修复（审查问题-高，问题6）

**审查意见**：v6→v7 修订记录（16.12 节）记载"依赖所有 iframe 页面同源部署的前提条件"，与当前 Vue 3 SPA 架构矛盾。

**修订**：
- 在第 16.12 节修订记录末尾追加说明：此前提条件描述已在 v1→v2 修订中随 iframe 架构移除而被替换，当前 4.4 节表单持久化采用 Pinia store + sessionStorage 方案，不再依赖 iframe 同源前提。

### 17.5 第 7.4 节部署架构术语统一（审查问题-中，问题9）

**审查意见**：第 7.4 节部署架构中"HTML/CSS/JS 静态文件"术语存在上下文歧义，在全文修订目标为消除原生开发描述的背景下存在歧义风险。

**修订**：
- 将第 7.4 节四处"HTML/CSS/JS 静态文件/HTML/CSS/JS 文件"统一修改为"Vite 构建产物（含 HTML/JS/CSS 静态资源）"。
- 同步将第 7.3 节可移植性描述中"Vite 构建产物为静态 HTML/CSS/JS 文件"更新为"Vite 构建产物（含 HTML/JS/CSS 静态资源）"。

### 17.6 前端技术栈全局统一修订

本项非审查报告的独立问题，而是本次修订的核心指令——将全文前端技术栈统一为 Vue 3 + TypeScript + Vite。除上述各节针对性修复外，还包括以下全局修订：

**第 8.1 节更新**：
- 将"Vue3 + Vite 构建工具实现"更新为"Vue 3 + TypeScript + Vite 构建工具实现"。
- 前端构建方式描述增加 TypeScript 的定位说明（静态类型检查、接口定义、编译期错误拦截）。
- 删除"CDN 外部化"表述（与 Vue 3 + Vite 工程的 npm 模块管理模式矛盾）。
- 前端源代码目录结构从 Vue3 + Vite 标准结构扩展为 Vue 3 + Vite + TypeScript 标准结构，新增 `src/types/`（TypeScript 类型定义）、`src/composables/`（组合式函数）、`src/utils/`（工具函数）、`vite.config.ts`、`tsconfig.json` 等条目。

**TypeScript 代码示例统一**：
- 第 1.1 节 Vue Router 路由配置示例：增强为完整 TypeScript 示例——加入 `RouteRecordRaw` 类型导入、`RouteMeta` 接口扩展声明、`routes` 数组的类型注解、`createRouter` 调用、带类型注解的全局导航守卫 `beforeEach` 实现。
- 第 4.10 节新增 Pinia authStore TypeScript 定义示例：使用 `defineStore` + Composition API（`ref`/`computed`），含 `UserInfo` 接口定义、`token: Ref<string | null>` 等类型注解、`setAuth`/`clearAuth` 方法的返回值类型定义。

### 本轮的跨节结构性变更

本轮修订涉及以下跨节的结构性调整：

- **第 1.1 节删除**：原 iframe 跨模块通信机制（含 postMessage 消息协议规范）整体删除，原 1.2 节"前端路由结构"重编号为 1.1 节。
- **第 1 节主文修订**：技术栈声明从前端"HTML、CSS、JavaScript"改为"Vue 3 + TypeScript + Vite"；架构描述从"各模块以 iframe 方式嵌入主框架"改为"所有模块运行在同一 Vue 运行时中，不使用 iframe 架构"。
- **第 4.10 节修订**：删除指向已删除第 1.1 节的交叉引用；新增 Pinia authStore TypeScript 代码示例。
- **第 7.3/7.4 节修订**：统一"HTML/CSS/JS 静态文件"术语为"Vite 构建产物（含 HTML/JS/CSS 静态资源）"。
- **第 8.1 节修订**：技术栈声明增加 TypeScript；删除 CDN 外部化表述；目录结构扩展为 TypeScript 标准结构。
- **第 9 节修订**：澄清说明第 1、11、12 条更新以反映 Vue 3 SPA 架构选择。
- **第 16.12/16.14 节修订**：在历史修订记录中追加 v2 修订说明，标注已被后续修订替换的内容。

---

## 18. 修订说明（v7 → v8）

本轮修订基于第 2 轮质量审查报告（`b_v2_diag_v2.md` 经质询确认）中提出的 8 个质量问题及 2 个持续存在的问题，逐一回应如下。8 个质量问题按严重程度分布：1 个严重问题（API 响应数据结构缺失）、5 个一般问题（时间指标测量起点、腰围/收缩压判定逻辑、SSE 连接管理、方案调整打卡记录处理、user_risk_info 表字段对应）、2 个轻微问题（marked.js 命名、Dify 备选方案细节）。2 个持续存在问题的解决也在本轮一并完成。

### 18.1 API 正常响应数据结构补充（审查问题-严重，问题1）

**审查意见**：第 6 节定义了 20+ 个 API 端点及其请求体参数，但几乎所有端点的正常响应数据结构（HTTP 200 response body JSON schema）均未定义，阻塞前后端并行开发。

**修订**：
- 在 6.1 节为 POST /api/auth/login 和 POST /api/auth/register 补充正常响应 JSON 结构和字段说明（token、role、user）。
- 在 6.3 节为 POST /api/risk/predict 补充正常响应 JSON 结构（risk_score、risk_level、risk_level_label、matched_diabetes_type、advice、record_id）及风险等级枚举值定义（low/medium/high）；为 GET /api/risk/history 说明采用分页响应外壳。
- 在 6.4 节为 GET /api/doctors 和 GET /api/doctors/:id 补充正常响应 JSON 结构和字段说明，并注明 chat_token 不在响应中返回。
- 在 6.5 节为 POST /api/plan/generate 补充正常响应 JSON 结构（plan_id、items 数组及其子字段）；说明 GET /api/plan/current 和 PUT /api/plan/adjust 的响应结构一致性。
- 在 6.7 节为 GET /api/articles（列表）和 GET /api/articles/:id（详情）分别补充正常响应 JSON 结构；说明 POST /api/articles/generate 返回结构与详情一致；补充 is_collected 字段说明。
- 在 6.9 节新增"SSE 事件 data 字段结构说明"段落，定义适用于所有 SSE 流式端点的 Dify 标准 SSE 事件格式（event、conversation_id、message_id、answer、created_at），在 6.4 节添加交叉引用。
- 所有新增响应结构均标注为基于数据表结构、功能逻辑或 Dify API 通用规范的推断，具体字段名和嵌套层级在概要设计阶段确定。

### 18.2 验收标准中的时间指标补充测量起点定义（审查问题-一般，问题2）

**审查意见**：4.2-4.9 节各验收标准中的时间指标（3 秒/10 秒/15 秒/1 秒）均未定义计时起点，7.2 节的"合理预期而非硬性约束"声明未交叉引用至各验收标准。

**修订**：
- 在 4.2 节验收标准中补充计时起点（用户点击发送按钮）和终点（首个 SSE 事件到达前端）定义，以及 DeepSeek API 负载影响声明。
- 在 4.4 节验收标准中补充计时起点（用户点击提交按钮）和终点（风险等级和建议渲染完成）定义。
- 在 4.5 节验收标准中补充计时起点（用户点击生成方案）和终点（第一条方案项渲染完成）定义。
- 在 4.6 节验收标准中补充计时起点（用户点击分类标签）和终点（文章正文渲染完成）定义。
- 在 4.8 节验收标准中补充计时起点（用户点击 FAB 按钮）和终点（对话窗口渲染完成）定义，并注明此指标为前端渲染性能指标。
- 在 4.9 节验收标准中补充计时起点（管理员发送消息）和终点（操作结果渲染完成）定义。
- 在 7.2 节性能要求中补充统一说明：各验收标准的时间指标以用户触发操作为计时起点，前端计时与服务端计时存在差异，依赖外部 AI 服务的指标受 API 负载和网络条件影响。

### 18.3 腰围/收缩压"未提供"判定逻辑澄清（审查问题-一般，问题3）

**审查意见**：用户在表单中输入 0 vs 留空的处理策略未定义。

**修订**：
- 在 4.4 节腰围/收缩压估算公式后新增"未提供判定规则"段落，明确 null/undefined/空字符串触发估算，数值 0 视为无效输入——前端表单校验拦截并提示"腰围/收缩压不能为 0，请填写有效值或留空"；服务端参数校验返回 VALIDATION_ERROR。前端输入框 placeholder 提示"留空将自动估算"。

### 18.4 多医生场景下 SSE 连接管理策略补充（审查问题-一般，问题4）

**审查意见**：用户切换医生时原医生 SSE 连接的生命周期未定义。

**修订**：
- 在 4.2 节"会话管理"段落后新增"SSE 连接管理"段落，明确：切换医生时前端应立即关闭当前 SSE 连接（AbortController.abort()）；仅当前可见对话窗口维持活跃 SSE 连接（同时连接数上限为 1）；后台医生对话的消息在用户切回时通过历史会话列表恢复。补充了移动端浏览器并发连接数限制和资源节约原则的技术理由说明。

### 18.5 方案调整后旧方案关联的打卡记录处理策略补充（审查问题-一般，问题5）

**审查意见**：方案调整时新方案"替换"旧方案，但旧方案关联的打卡记录通过 plan_id 外键关联——物理删除将导致打卡记录 plan_id 悬空。

**修订**：
- 在 4.5 节"方案调整"段落重写为逻辑过期策略：life_plans 表新增 is_active 字段（BOOLEAN，默认 TRUE），方案调整时标记旧方案为 FALSE、新方案为 TRUE；打卡分析默认仅统计活跃方案项，历史分析可包含非活跃记录。
- 在 5 节 life_plans 表定义中新增 is_active 字段说明，并在 6.5 节 POST /api/plan/generate 处理流程描述中同步引用逻辑过期机制。
- 补充了此策略的数据关系完整性约束理由。

### 18.6 user_risk_info 表字段与风险预测 API 参数对应修复（审查问题-一般，问题6）

**审查意见**：diabetes_history（API 必填参数）在 user_risk_info 表中无直接对应字段；4.6 节健康资讯数据源描述中引用了表中不存在的"BMI"字段。

**修订**：
- 在 5 节 user_risk_info 表定义中新增 diabetes_history 字段（TEXT），并注明与 POST /api/risk/predict 的 diabetes_history 参数对应。
- 在 4.6 节健康资讯数据源优先级描述中将"BMI"修正为"身高和体重（用于计算 BMI）"。

### 18.7 前端 npm 包名 marked.js 修正为 marked（审查问题-轻微，问题7）

**审查意见**：npm 官方包名为 `marked`，文档中两处使用 `marked.js`（第 1 轮已识别但未修正）。

**修订**：
- 将 4.6 节（第 291 行）"marked.js"修正为"marked"。
- 将 8.1 节（第 992 行）"marked.js"修正为"marked"。

### 18.8 Dify 平台能力验证备选方案补充关键实现细节（审查问题-轻微，问题8）

**审查意见**：备选方案的 session_id 传递机制与主方案面临相同的 `{{user}}` 变量透传依赖；映射表的生命周期管理和存储位置未覆盖。

**修订**：
- 在 5 节"Dify 平台能力验证任务"备选方案描述中补充三项关键约束：（1）session_id 传递仍依赖 `{{user}}` 变量透传——若 Dify 平台不支持，主方案和备选方案均无法工作，需进一步退化为方案 B（管理员手动输入 user_id 或在 Agent 提示词中写死测试用户 ID）；（2）映射表存储位置推荐 Express 进程内存 Map，备选 SQLite 持久化方案；（3）映射表生命周期——用户主动登出和 JWT 过期时清理，清理周期与 JWT_EXPIRES_IN 一致。

### 18.9 chatStore 和 riskFormStore 的 TypeScript 类型定义补充（持续存在问题，第1轮遗留）

**审查意见**：第 1 轮审查指出 TypeScript 代码示例覆盖不足（包括 authStore 和 chatStore），authStore 已在第 2 轮中获得完整 TS 定义，但 chatStore 和 riskFormStore 仍仅有自然语言描述。

**修订**：
- 在 4.10 节 authStore 定义后新增"Pinia chatStore 类型定义"子节，含完整 TypeScript 代码示例——使用 defineStore + Composition API，定义 doctorConversations（Map<number, string>）、assistantConversationId（Ref<string | null>）两个核心状态，以及 setDoctorConversation/getDoctorConversation/setAssistantConversation/clearDoctorConversation/clearAssistantConversation 方法，均含类型注解。补充了 Map 类型在 Pinia 持久化插件中的序列化注意事项。
- 在 4.10 节新增"Pinia riskFormStore 类型定义"子节，含完整 TypeScript 代码示例——定义 RiskFormData 接口（含全部 10 个字段及其类型和可选性）、loadFromStorage/updateFormData/clearFormData 方法、sessionStorage 持久化读写逻辑。补充了 sessionStorage JSON 解析错误处理和页面刷新自动恢复的说明。

### 本轮的跨节结构性变更

本轮修订涉及以下跨节的结构性调整：

- **第 4.2 节新增段落**："SSE 连接管理"。
- **第 4.4 节新增段落**："未提供判定规则"。
- **第 4.5 节重写段落**："方案调整"从单句描述扩展为逻辑过期机制完整说明。
- **第 4.6 节字段名修正**："BMI"修正为"身高和体重（用于计算 BMI）"。
- **第 4.10 节新增两个子节**："Pinia chatStore 类型定义"和"Pinia riskFormStore 类型定义"，含完整 TypeScript 代码示例。
- **第 5 节数据表定义更新**：user_risk_info 表新增 diabetes_history 字段；life_plans 表新增 is_active 字段；Dify 平台能力验证备选方案扩展为三项约束说明。
- **第 6.1 节新增段落**：POST /api/auth/login 和 POST /api/auth/register 正常响应结构。
- **第 6.3 节新增段落**：POST /api/risk/predict 正常响应结构（含风险等级枚举值定义）。
- **第 6.4 节新增段落**：GET /api/doctors 和 GET /api/doctors/:id 正常响应结构；SSE 事件结构交叉引用。
- **第 6.5 节新增段落**：POST /api/plan/generate 正常响应结构。
- **第 6.7 节新增段落**：GET /api/articles、GET /api/articles/:id 和 POST /api/articles/generate 正常响应结构。
- **第 6.9 节新增段落**："SSE 事件 data 字段结构说明"（适用于所有 SSE 流式端点）。
- **第 7.2 节补充说明**：时间指标统一计时起点定义和 AI 服务依赖声明。
- **第 8.1 节命名修正**：marked.js → marked。
- **第 4.6 节命名修正**：marked.js → marked。

---

## 19. 修订说明（v8 → v8_rev1）

本轮修订基于第 4 轮审查报告（`a_v4_review_v1.md`）中提出的 8 个未解决问题，分类逐一回应如下。8 个问题按严重程度分布：0 个严重问题（2 个严重问题已在上一轮修订中解决，本轮审查确认通过）、6 个一般问题、3 个轻微问题（其中 1 个轻微问题与一般问题合并讨论，实际需解决的问题为 8 个）。

### 19.1 枚举字段字符串值全文统一约定（审查问题-一般，第3轮问题7 / 本轮诊断编号7）

**审查意见**：diabetes_history、gender、family_history 等枚举字段在 4.4 节（中文自然语言 + "如"字暗示为示例）、6.3 节（"如"字示例）、5 节（无约束说明）三处描述精度不一致，前后端开发者无法独立确定枚举值的完整集合和字符串表示形式。

**修订**：
- 在 4.4 节病史状态选择步骤中明确枚举值字符串：diabetes_history 为 `"healthy"` / `"prediabetes"` / `"diagnosed"`；diabetes_type 为 `"type1"` / `"type2"` / `"gestational"` / `"other"`。标注"其他值将被服务端拒绝"。
- 在 6.3 节 POST /api/risk/predict 请求体参数表中为 diabetes_history、diabetes_type、gender、family_history 四个字段逐一补充完整枚举值定义和"其他值将被服务端拒绝"声明。
- 在 5 节 user_risk_info 表定义中为 gender、family_history、diabetes_history、diabetes_type 字段补充 CHECK 约束。
- 同步为 5 节 life_plans 表（plan_type → `'diet'`/`'exercise'`/`'other'`）和 punch_in 表（punch_type → `'diet'`/`'exercise'`；completion_status → `'completed'`/`'uncompleted'`）补充 CHECK 约束和枚举值说明。
- 在 6.6 节 POST /api/punch 请求体参数表中为 punch_type 和 completion_status 补充枚举值定义。

### 19.2 健康建议触发机制标注为推断（审查问题-轻微，本轮诊断新发现）

**审查意见**：4.8 节健康建议触发机制的描述（"由 Dify Agent 根据对话上下文自主决策"）未标注为推断性补充，下游架构师可能将其误解为经过验证的确定行为。

**修订**：
- 在 4.8 节"健康建议的触发机制"段落标题后补充"（基于 Agent 模式通用特性的推断，具体触发行为以 Dify Agent 实际配置为准）"标注。

### 19.3 修订说明章节阅读顺序优化（审查问题-一般，第3轮问题5 / 本轮诊断编号5）

**审查意见**：第 17 节（v1→v2，最早修订批次）被插入在第 16 节（v6→v7）和第 18 节（v7→v8）之间，破坏时间线一致性，实现者按章节顺序阅读会产生"前面刚定义、后面又删除"的困惑。

**修订**：
- 在修订说明区域开头（原第 10 节之前）新增"修订说明阅读指引"段落，以表格形式列出按时间线从旧到新的 9 个修订批次的阅读顺序、章节编号和内容概要。
- 在第 17 节（v1→v2）开头新增阅读提示块（blockquote），明确标注"本节为全部修订批次中时间线最早的一批，按时间线阅读时应优先阅读本节"。
- 阅读指引表格中各节的"章节"列标注了各修订批次在文档中的实际物理位置，读者可通过表格导航至任意批次。

### 19.4 Dify Agent 预期工具清单补充（审查问题-一般，第3轮问题6 / 本轮诊断编号6）

**审查意见**：Agent 挂载的工具链决定 AI 助手和智能管理的功能边界，当前在需求阶段将所有 Agent 工具定义推迟至概要设计阶段，前后端开发者进入概要设计时仍需返回需求层面做大量澄清。

**修订**：
- 在 6.11 节 Dify 工作流清单汇总表说明中新增两个 Agent 的"预期工具清单"表格：
  - diabetes-assistant-agent（AI 智能助手）：列出 query_user_profile、query_risk_history、query_punch_records、query_life_plans、query_health_advice、write_health_advice、update_user_profile、knowledge_search 共 8 个预期工具，每个工具标注用途、输入输出类型、权限约束和前端交互影响。
  - admin-manager-agent（智能管理）：列出 query_table、insert_record、update_record、delete_record、get_table_schema 共 5 个预期工具，每个工具标注用途、输入输出类型、权限约束和前端交互影响。
- 各工具的"前端交互影响"列标注了前端需实现的特殊交互行为（二次确认弹窗、操作反馈气泡、结构化结果渲染等），供前端开发者在概要设计阶段据此设计对话消息渲染组件。
- 标注以上工具清单为"基于功能需求和 Agent 通用设计模式的推断"，完整工具定义和参数 schema 在概要设计阶段确定。

### 19.5 SSE 事件格式交叉引用链简化（审查问题-轻微，第3轮问题9 / 本轮诊断编号9）

**审查意见**：SSE 事件结构的引用关系为"第 6.10 节 → 第 6.4 节 → 第 6.9 节"，形成两跳引用链，增加实现者查找成本。

**修订**：
- 将第 6.10 节（管理员对话）的 SSE 代理转发策略说明中补充直接引用"SSE 事件 data 字段结构说明见 6.9 节——该结构适用于所有 SSE 流式端点"，消除经 6.4 节的中间跳转。
- 第 6.4 节原有的直接引用 6.9 节保持不变（6.4 节在上一轮修订中已标注 6.9 节"适用于所有 SSE 流式端点"）。

### 19.6 移动端 UI 组件库选型补充（审查问题-一般，第3轮问题8 / 本轮诊断编号10）

**审查意见**：文档明确平台为"移动端优先的 Web 应用"，定义了完整的设计系统基础参数，但全文未涉及移动端 UI 组件库选型，导致移动端特有交互组件（Tabbar、ActionSheet、DatetimePicker、PullRefresh 等）的开发工作量和工作方式不确定。

**修订**：
- 在 8.1 节前端构建方式描述中新增"移动端 UI 组件库选型"段落：推荐采用 Vant 4 作为移动端优先的 Vue 3 组件库，说明其提供的移动端特有组件、Tree Shaking 按需引入、内置主题定制能力（CSS Variables）等关键特性。同时声明若项目组有意推迟选型至概要设计阶段，需评估移动端适配完整性、Vue 3 + TypeScript 兼容性、Bundle 体积、社区活跃度四个维度后确定。
- 在 7.1 节设计系统参数末尾补充组件库主题变量映射说明：若采用 Vant 4，设计系统的 CSS 变量应与 Vant 4 主题变量体系（如 `--van-primary-color` 等）建立映射关系，具体映射表在概要设计阶段确定。

### 19.7 验收标准时间指标免责声明统一（审查问题-轻微，第3轮问题8 / 本轮诊断编号8）

**审查意见**：各验收标准中免责声明长度不一——部分指标后附加较长免责声明文本，部分仅简短标注，增加了验收标准的解释歧义。

**修订**：
- 将 4.2 节、4.4 节、4.5 节、4.6 节、4.9 节共 5 处验收标准中的时间指标免责声明统一为简短模板："此指标为合理预期而非硬性约束，实际值受 [具体影响因素] 影响，详见 7.2 节"。
- 删除各验收标准中重复的大段免责文本（如"基于 DeepSeek API 一般响应时间的推断"等），统一通过交叉引用指向 7.2 节的统一定义。

### 19.8 localStorage 跨浏览器标签页登录态同步策略补充（审查问题-轻微，第3轮问题11 / 本轮诊断编号11）

**审查意见**：多标签页场景下，一个标签页登出后其他标签页的 Pinia store 不会自动感知此变更，可能导致标签页 B 仍显示已登录状态但实际 API 调用返回 401。

**修订**：
- 在 4.10 节新增"跨浏览器标签页的登录态同步"段落，定义同步策略：应用初始化时监听 `window.addEventListener('storage', ...)` 事件，当检测到 `token` 或 `role` 键在其他标签页中被修改或移除时，同步更新当前标签页 Pinia authStore 状态（若 token 被移除则调用 `authStore.clearAuth()` 触发登出；若 token 被修改则同步更新）。
- 标注跨标签页同步的具体实现（事件监听器注册时机、防抖策略、与路由守卫的协作）在概要设计阶段确定。

### 本轮的跨节结构性变更

本轮修订涉及以下跨节的结构性调整：

- **第 4.4 节更新**：病史状态选择和糖尿病类型选择的枚举值统一为英文字符串约定。
- **第 4.8 节更新**：健康建议触发机制段落标题补充推断标注。
- **第 4.10 节新增段落**："跨浏览器标签页的登录态同步"。
- **第 5 节数据表定义更新**：user_risk_info、life_plans、punch_in 三个表补充 CHECK 约束和枚举值说明。
- **第 6.3 节参数表更新**：diabetes_history、diabetes_type、gender、family_history 四个参数补充完整枚举值定义。
- **第 6.6 节参数表更新**：punch_type、completion_status 补充枚举值定义。
- **第 6.10 节更新**：SSE 代理转发策略中补充对 6.9 节的直接 SSE 事件结构引用。
- **第 6.11 节新增段落**：diabetes-assistant-agent 和 admin-manager-agent 的预期工具清单表格。
- **第 7.1 节更新**：设计系统参数末尾补充组件库主题变量映射说明。
- **第 7.2 节更新**：验收标准时间指标免责声明措辞统一（通过各功能章节间接体现）。
- **第 8.1 节新增段落**：移动端 UI 组件库选型（Vant 4 推荐及评估维度）。
- **第 4.2/4.4/4.5/4.6/4.9 节更新**：5 处验收标准时间指标免责声明统一为简短模板 + 7.2 节交叉引用。
- **新增"修订说明阅读指引"**：位于原第 10 节之前，含时间线阅读顺序表格。
- **第 17 节新增阅读提示**：blockquote 标注为最早修订批次。
