...
> **诊断日期**: 2026-06-27
> **版本**: v5（v2修订版，响应 v5 审查反馈 + v5 质询反馈）
...
|---------|:------:|:------:|:---:|
| 确认（代码偏离设计） | 11 | 27 | 38 |
| 设计对齐（代码符合设计，但设计本身或观察成立） | 2 | 1 | 3 |
| 部分确认（问题存在但结论有细化空间） | 0 | 0 | 0 |
| 不成立（代码实现优于设计规定，无需修改代码） | 0 | 1 | 1 |

> **注**: G5（LifePlan打卡弹窗交互顺序）诊断结论为"不成立——代码实现优于设计文档规定"，仅需设计文档更新。S5b-1（chatStore SSE核心实现）诊断结论为"确认——chatStore为骨架实现，需从头构建SSE通信层"，该条目为本轮新拆分子任务（原S5b拆分为S5b-1和S5b-2），详见S5b条目。无需代码修改的条目共计4项（G4、G5、S12、S13），详见各条目。"不成立"行计入结论标签为"不成立"的条目（G5）；G4、S13 归类为"设计对齐"（代码符合设计），S12 归类为"确认"（问题存在但无需代码修改）。**注意**：G4 条目头部诊断结论标签已从 v4 的"部分确认"修正为"设计对齐"——G4 的诊断本质为代码符合设计（设计文档在不同位置分别描述了按钮和无限滚动两种模式，双模式均有设计依据），与汇总表归类一致。
...
**S5a — ArticleDetailView.vue + /news/article/:id 路由**（复杂度: 低至中，取决于后端 API 就绪状态）

- **设计依据**: 1.6.1节路由映射表（第432行）：`/news/article/:id → ArticleDetailView.vue`；1.6.2节路由守卫代码示例（第477-480行）：`meta: { requiresAuth: false }`
- **代码证据**: `src/router/index.ts:5-67` 无此路由；文件系统中不存在 `ArticleDetailView.vue`；`src/composables/useHomeApi.ts` 当前仅含 `getArticles()`（列表），不存在 `getArticle(id)` 单篇详情函数。
...
- **修复建议**:
  ...
  - **可复用模式**: `src/views/LifePlan.vue:94-99` 的 `safeContentHtml` 模式；`src/composables/useHomeApi.ts` 的 API 调用模式；**特别参考** `useHomeApi.ts:67-72` 的 `getDiabetesType(id)` 函数——该函数实现了按ID获取单个资源详情的完整模式（`GET /api/resource/:id` + 路由参数解析 + 错误处理 + 类型标注），可直接作为 `getArticle(id)` 的实现模板，加速正向实现。
  - **边界条件**: 文章 ID 不存在时展示 404 提示（非路由级 404，组件内降级）；正文为空时显示占位文案；API 失败时展示重试按钮。
- **验证方法**: 直接访问 `/news/article/1`（假设存在此 ID），检查页面是否正常渲染文章标题、正文、元信息；访问 `/news/article/99999`（不存在ID），检查组件内降级展示。执行 `vue-tsc --noEmit` 确认无新增编译错误。

**S5b — DoctorChatView.vue + /consultation/doctor/:id 路由**

	- **诊断结论**: **确认 — 代码偏离设计，且核心依赖 chatStore 仅为骨架实现（见下方拆分）**
	- **严重程度**: 高（与 todo.md 一致，但需拆分为两个子任务以如实反映 chatStore 当前状态）
	- **代码验证结果**: 经实际读取 `src/stores/chatStore.ts` 全文（共13行），确认 chatStore 当前为骨架实现：
	  - `conversations` 为 `ref([])`（空数组，无任何对话数据）
	  - `abortActiveConnection()` 函数体为空（`{}`），不含任何 SSE 连接终止逻辑
	  - `clearAllConversations()` 仅执行 `conversations.value = []`
	  - 文件内无任何 `EventSource`、`fetch`、`WebSocket`、`SSE` 相关代码
	  - `src/types/sse.ts` 中定义的 `SSEMessageEvent`、`SSEErrorEvent`、`ChatMessage` 等类型完全未被 chatStore 引用（无 import 语句）
	- **结论**: S5b 的"依赖 chatStore 和 SSE 机制就绪"假设不成立——chatStore 不是"待集成"状态，而是"需从头构建"状态。必须将 S5b 拆分为两个独立子任务，S5b-1 为 S5b-2 的硬前置依赖。

	**S5b-1 — chatStore SSE 核心实现**（复杂度: 高，独立子任务，S5b-2 硬前置依赖）

	- **诊断结论**: **确认 — chatStore为骨架实现，需从头构建SSE通信层（conversations空数组、abortActiveConnection空函数体、不含任何EventSource/SSE代码）**
	- **现状**: `src/stores/chatStore.ts` 仅有骨架（conversations 空数组、abortActiveConnection 空函数体），需从头构建完整的 SSE 通信层。
	- **设计依据**: 1.6.1节路由映射表（第429行）定义了 `/consultation/doctor/:id` 路由，隐含 chatStore 提供对话管理能力；4.2节状态管理表（第3476行）：`chatStore | Pinia | 对话状态管理`。
	- **影响范围**: 医生对话功能完全不可用（chatStore 是所有对话相关功能的核心数据层）。
	- **修复建议**:
	  - **修改文件**: `src/stores/chatStore.ts` — 重写完整实现
	  - **关键逻辑**: (a) SSE 连接管理：使用 `EventSource` 创建 `/api/chat/stream?conversation_id=xxx` 连接，监听 `message`/`message_end`/`error` 事件（对应 `src/types/sse.ts` 中 `SSEMessageEvent`/`SSEMessageEndEvent`/`SSEErrorEvent` 类型）；(b) 消息收发：`sendMessage(doctorId: number, content: string)` action 通过 `POST /api/chat/send` 发送用户消息，`onmessage` 回调中将 `SSEMessageEvent.answer` 追加到当前对话消息列表；(c) `conversation_id` 管理：`fetchOrCreateConversation(doctorId: number)` 通过 API 获取或创建对话，维护 `activeConversationId` ref；(d) 多医生会话路由：根据 `doctorId` 参数路由到对应会话，支持同一用户与不同医生的独立对话；(e) `fabOpen` 状态管理：控制移动端 Fab 按钮展开/收起；(f) `abortActiveConnection()` 实现：关闭当前 EventSource 连接，在组件卸载或切换医生时调用；(g) 断线重连：`onerror` 事件中实现指数退避重连（1s → 2s → 4s → max 30s），重连前检查 token 有效性。
	  - **边界条件**: 用户未登录时 SSE 连接不建立（token 缺失）；对话历史为空时从服务端加载（`onMounted` 调用 `fetchConversationHistory(conversationId)`）；消息发送中禁用发送按钮（`sending` ref 防双击）；SSE 流式消息渲染需支持 Markdown 片段实时渲染（`marked.parse` 增量模式或按换行分割渲染）。
	- **验证方法**: 在 DevTools Network 面板中确认 `/api/chat/stream` 的 EventSource 连接成功建立（Content-Type: text/event-stream）；发送消息后确认 SSE 事件流中 `message` 事件的 `answer` 字段被正确追加到消息列表；断开网络后确认重连机制启动；切换浏览器标签页后确认连接状态恢复。执行 `vue-tsc --noEmit` 确认无新增编译错误。

	**S5b-2 — DoctorChatView.vue 组件 + /consultation/doctor/:id 路由注册**（复杂度: 中高，硬依赖 S5b-1）

	- **诊断结论**: **确认 — 组件文件缺失且路由未注册，硬依赖S5b-1（chatStore SSE核心实现必须先完成）**
	...
	- **影响范围**: 医生对话 UI 不可达（即使 S5b-1 完成 chatStore 实现，仍缺少入口）。
	- **前置依赖**: S5b-1（chatStore SSE 核心实现）必须先完成——DoctorChatView.vue 的所有对话交互（发送消息、显示消息列表、SSE 流式渲染）均依赖 chatStore 的 `conversations`/`sendMessage`/`abortActiveConnection` 等接口。
	- **修复建议**:
	  - **需创建文件**: `src/views/DoctorChatView.vue`
	  - **需修改文件**: `src/router/index.ts` — 添加路由配置；`src/views/Consultation.vue` — 医生卡片添加点击跳转逻辑（`router.push('/consultation/doctor/' + id)`）
	  - **关键逻辑**: (a) 组件接收 `route.params.id`，在 `onMounted` 中调用 API 获取医生信息（姓名、科室、职称、头像）；(b) 集成 `chatStore`：调用 `chatStore.fetchOrCreateConversation(doctorId)` 初始化会话，渲染 `chatStore.conversations` 中的消息列表（用户消息 + AI 回复流式渲染）；(c) 输入框发送消息：调用 `chatStore.sendMessage(doctorId, inputContent)`；(d) 路由 `meta: { requiresAuth: true, requiresDisclaimer: true }`；(e) `onUnmounted` 中调用 `chatStore.abortActiveConnection()` 关闭 SSE 连接。
	  - **边界条件**: SSE 连接中断时展示重连按钮（调用 `chatStore.reconnect()`）；医生 ID 不存在时展示错误态；对话历史加载失败时展示重试态；`requiresDisclaimer: true` 触发路由守卫的免责声明弹窗。
	- **验证方法**: 从 Consultation 页点击医生卡片跳转至 `/consultation/doctor/1`，检查页面是否显示医生信息；发送消息后检查 SSE 流式渲染是否正常；检查免责声明弹窗是否在首次访问时触发；快速切换医生（/consultation/doctor/1 → /consultation/doctor/2）检查旧 SSE 连接是否被正确关闭（无僵尸连接）。执行 `vue-tsc --noEmit` 确认无新增编译错误。
...
- **修复建议（短期过渡）**:
  ...
  - **跨标签页同步（可选增强）**: sessionStorage 迁移后，每个新标签页因缺失 token 将强制跳转登录页，形成 UX 退化。以下为 BroadcastChannel 最小实现方案（标注为可选增强，核心修复可独立交付）：
    ```typescript
    // 在 authStore.ts 中 — BroadcastChannel 懒初始化（避免模块顶层副作用）
    let channel: BroadcastChannel | null = null
    function getChannel(): BroadcastChannel | null {
      if (channel) return channel
      try { channel = new BroadcastChannel('qrzl_auth_sync') } catch { channel = null }
      return channel
    }
    // 在 setAuth() / clearAuth() 末尾广播（通过 getter 按需创建）：
    getChannel()?.postMessage({ type: 'AUTH_CHANGED', timestamp: Date.now() })
    // onmessage 处理器在 authStore 初始化时注册一次（仅在 getChannel() 首次创建时）：
    if (getChannel()) {
      getChannel()!.onmessage = (e: MessageEvent) => {
        // 状态就绪检查：确认 token 和 role ref 已初始化后再同步
        if (e.data?.type === 'AUTH_CHANGED' && token && role) {
          syncFromStorage()
        }
      }
    }
    ```
    - **变更说明（v5修订）**: 将 `new BroadcastChannel(...)` 从模块级 IIFE 改为 `getChannel()` 懒初始化函数：(a) 避免模块顶层执行 `new BroadcastChannel(...)` 在单元测试/SSR 环境（BroadcastChannel API 不存在）时抛出异常；(b) `onmessage` 处理器增加 `if (token && role)` 状态就绪检查，防止 Pinia Store ref 未完全初始化时收到消息并调用 `syncFromStorage()` 导致中间态污染。
    - 实现要点：(a) `setAuth()` 和 `clearAuth()` 末尾调用 `channel?.postMessage(...)`，通知其他标签页 token 变更；(b) 收到 `AUTH_CHANGED` 消息时调用 `syncFromStorage()` 从 sessionStorage 同步最新 token；(c) 新标签页首次加载时 `syncFromStorage()` 因 sessionStorage 为空而进入未登录状态，待用户登录后通过 BroadcastChannel 同步到所有标签页；(d) 若浏览器不支持 BroadcastChannel（`catch` 返回 null），静默降级——跨标签页不自动同步，用户需在新标签页手动重新登录。
    - **明确标注**: 此方案为可选增强，S8 核心修复（localStorage -> sessionStorage 迁移）可独立交付。不做跨标签页同步不会导致数据错误——仅要求用户在新标签页重新登录，与当前 sessionStorage 设计目标一致。
...
- **验证方法**: 在 LifePlan 方案内容中尝试注入 `<img src=x onerror=alert(1)>`，检查渲染后 onerror 是否被移除；在 Punch 分析评语中尝试注入 `<a href="javascript:alert(1)">click</a>`，检查 href 是否被移除或替换为 `#`。执行 `vue-tsc --noEmit` 确认无新增编译错误。
...
- **诊断结论**: **设计对齐 — 双模式均有设计依据，代码并存提供双保险用户体验**
...
- **验证方法**: 检查 LifePlan 方案内容渲染和 Punch AI 分析渲染是否正常（Markdown→HTML 管道功能不变）。执行 `vue-tsc --noEmit` 确认无新增编译错误。
...
- **验证方法**: 在 LifePlan 和 Punch 中触发 API 错误（如断网），检查错误文案是否正确显示。执行 `vue-tsc --noEmit` 确认无新增编译错误。
...
- **验证方法**: 删除 Home.vue 本地接口定义后，TypeScript 编译无报错（证明 import 的接口已覆盖所有使用）。合并前需检查两处接口定义是否完全相同——若有字段差异，需先统一字段再合并。执行 `vue-tsc --noEmit` 确认无新增编译错误。
...
- **验证方法**: 在糖尿病类型弹窗中检查病因/临床表现/治疗方式的文本是否正确转义（如含 `<` 的文本不被渲染为 HTML 标签）。执行 `vue-tsc --noEmit` 确认无新增编译错误。
...
- **修复建议**:
  ...
  - **关键逻辑**: (a) 方案A（推荐）: 不传递 cancel token 到每个 API 调用，而是在 Store 的 action 中使用 `requestId` 竞态快照（punchStore 已有此模式）来丢弃组件卸载后的响应——这比 AbortController 更简单且已部分实现；(b) 方案B: 如果仍希望使用 AbortController，在各 View 组件的 `onUnmounted` 中调用 `abort()` 取消该组件触发的所有进行中请求；(c) 方案A 优先——S9 修复后 punchStore 的三个 action（`fetchList`/`loadMore`/`fetchAnalysis`）均有 requestId 快照保护，但 homeStore 和 lifePlanStore 的 action 尚未覆盖，G18 需将同一 requestId 快照模式扩展到以下 action：`homeStore.fetchHomeData()`（并行三请求）、`lifePlanStore.fetchCurrent()`、`lifePlanStore.generate()`、`lifePlanStore.adjust()`。**与 S9 的关系**: S9 为 punchStore 的三个 action 实现了 requestId 快照模式，G18 需将该模式扩展到 homeStore 和 lifePlanStore——两者的实现模式完全相同（requestId ref 递增 + snapshot 快照 + finally 检查），S9 可作为 G18 的参考模板。S9 完成后，punchStore 的竞态保护已完备；G18 完成后，全部三个 Store 的异步 action 均受 requestId 快照保护。**建议修复顺序**: S9 先于 G18，S9 为 G18 提供已验证的 requestId 快照模板。
...
- **S5a（ArticleDetailView组件+路由）**: 需新建一个 Vue 组件文件并注册路由。涉及 marked+DOMPurify 渲染，与 LifePlan 内容渲染模式一致，技术路径明确。复杂度低至中，取决于后端 API 就绪状态——useHomeApi.ts 当前仅含 getArticles（列表），不存在 getArticle(id) 单篇文章详情函数，需确认后端 GET /api/articles/:id 接口是否已存在。若后端接口已就绪则复杂度为低；若需新增则复杂度为中。
- **S5b（DoctorChatView组件+路由）**: 经实际代码验证，`src/stores/chatStore.ts` 当前为骨架实现（conversations 空数组、abortActiveConnection 空函数体），不含任何 SSE/EventSource 实现代码。`src/types/sse.ts` 中 SSEMessageEvent/SSEErrorEvent/ChatMessage 等类型已就绪但未被 chatStore 引用。S5b 已拆分为两个子任务：(a) S5b-1（chatStore SSE 核心实现，复杂度高）——需从头构建 EventSource 连接管理、消息收发、conversation_id 管理、断线重连、多医生会话路由等完整 SSE 通信层；(b) S5b-2（DoctorChatView.vue 组件 + 路由注册，复杂度中高）——依赖 S5b-1 的 chatStore 接口。两者均无阻塞性技术障碍——EventSource 为浏览器原生 API，SSE 协议标准成熟，路由注册模式与现有 S5a 相同。
...
**受影响问题**: S5a, S5b, S6（路由和组件缺失）→ 交叉引用 S6 依赖 S5a；S5b 拆分为 S5b-1（chatStore SSE 核心）和 S5b-2（DoctorChatView.vue 组件）

设计文档v13修订时（第6834-6858行）将 Consultation 拆分为 Consultation.vue + DoctorChatView.vue，News 拆分为 NewsView.vue + ArticleDetailView.vue。修订更新了1.6.1路由表和组件树。实现阶段：(a) 两个新增组件文件（DoctorChatView.vue、ArticleDetailView.vue）及对应路由配置均未创建；(b) 核心依赖 chatStore 仅实现了骨架（conversations 空数组、abortActiveConnection 空函数体），导致 DoctorChatView.vue 即使创建也无法工作——chatStore 需从头构建 SSE 通信层。
...
- S5b-1（chatStore 为骨架实现）：医生对话功能完全不可用——chatStore 不含任何 SSE/EventSource 实现，需从头构建
- S5b-2（DoctorChatView 路由/组件缺失）：即使 S5b-1 完成，仍缺少 UI 入口

**高危级（影响核心体验但功能可降级运行）**:
- S1（首页缓存缺失）：性能退化，每次加载3次API请求
- S2（方案缓存缺失）：页面刷新后方案丢失
- S6（文章跳转错误）：用户无法查看文章详情 — 前置依赖 S5a
- S8（Token 明文存储安全隐患）：XSS 窃取风险影响用户会话安全性 — 当前 localStorage 方案功能正常，安全性不足，需迁移至 sessionStorage

**中危级（功能部分可用但有偏差）**:
- S3, S4, S7, S9, S10, S11, G3, G6, G14, G24
...
|:------:|------|:-----:|:------:|---------|-----------|------|
| **P0** | S5a | 高 | 代码修复 | 待确认：后端 GET /api/articles/:id 接口是否存在 [†D] | — | ArticleDetailView.vue + 路由，需确认后端 API 就绪状态；后端 API 不可用时可用 getArticles() 降级方案 |
| **P0** | S5b-1 | 高 | 代码修复 | 无（chatStore 骨架已存在，需重写） | — | chatStore SSE 核心实现：EventSource 连接管理、消息收发、conversation_id 管理、断线重连、多医生会话路由 [†E] |
| **P1** | S5b-2 | 高 | 代码修复 | **→ S5b-1** | — | DoctorChatView.vue + 路由注册，硬依赖 S5b-1 chatStore 接口 [†E] |
| **P1** | S6 | 中 | 代码修复 | **→ S5a** | — | 文章跳转修复依赖 ArticleDetailView 路由存在 |
| **P1** | S1, S2 | 高 | 代码修复 | 无 | **批处理 S1+S2** | 同为 sessionStorage 缓存模式，用同一模板实现 [†B] |
| **P1** | S9 | 中 | 代码修复 | 无 | — | fetchAnalysis 竞态保护——建议在 S3/S7 之前完成（S7 修复建议明确要求 fetchAnalysis 需先有竞态保护）[†A] |
| **P1** | S3, S7 | 中 | 代码修复 | S9（建议先完成）→ S3/S7 的 fetchAnalysis 调用需竞态保护 | **批处理 S3+S7** | 同在 Punch.vue + punchStore.ts，修改区域重叠。**注意**: 建议在 S9 完成后再执行 S3/S7 |
| **P2** | S4, S11 | 中/低 | 代码修复 | 无 | **批处理 S4+S11** | P2 内部排序 #1：同在 LifePlan.vue onMounted + computed，修改区域重叠 |
| **P2** | G14 | 中 | 代码修复 | S9（建议）→ G14 方案B 在 S9 完成 requestId 快照后更安全 [†A] | — | P2 内部排序 #2：API 响应拦截器统一处理 success 字段检查，影响面最广（3个文件10个函数）[†A][†C] |
| **P2** | G18 | 中 | 代码修复 | S9（requestId 快照模板）→ G18 将同一模式扩展到 homeStore/lifePlanStore [†F] | — | P2 内部排序 #3：AbortController/requestId 快照扩展——S9 已为 punchStore 实现模板，G18 需将该模式扩展到 homeStore.fetchHomeData()、lifePlanStore.fetchCurrent()/generate()/adjust() |
| **P2** | S8 | 中 | 代码修复 | S1/S2（建议） | — | P2 内部排序 #4：sessionStorage 迁移，涉及 authStore + 路由守卫联动 [†B] |
...
> - **[†D]**: S5a 依赖后端 GET /api/articles/:id 接口。当前 useHomeApi.ts 仅含 getArticles（列表），不存在 getArticle(id) 单篇详情函数。修复前需确认后端接口是否已就绪。若后端接口未就绪，可使用降级方案：先用 `getArticles()` 全量拉取后在客户端 `find(a => a.id === Number(id))` 筛选（详见 S5a 修复建议中的"后端 API 不可用时的降级方案"）。
> - **[†E]**: S5b-1 ↔ S5b-2 为硬依赖关系。S5b-1（chatStore SSE 核心）必须先完成，S5b-2（DoctorChatView.vue 组件）的所有对话交互（发送消息、消息列表渲染、SSE 流式展示）均依赖 chatStore 暴露的 `conversations`/`sendMessage`/`abortActiveConnection`/`fetchOrCreateConversation` 等接口。S5b-2 的 `onUnmounted` 需调用 `chatStore.abortActiveConnection()`，S5b-1 必须先实现该函数。
> - **[†F]**: G18 ↔ S9 共享 requestId 快照模式。S9 为 punchStore 的三个 action（fetchList/loadMore/fetchAnalysis）建立了 requestId 快照模板，G18 需将同一模板扩展到 homeStore（fetchHomeData 并行三请求）和 lifePlanStore（fetchCurrent/generate/adjust）。S9 完成后 punchStore 竞态保护完备，G18 在此基础上补全 homeStore 和 lifePlanStore。具体需保护的 action 列表：`homeStore.fetchHomeData()`、`lifePlanStore.fetchCurrent()`、`lifePlanStore.generate()`、`lifePlanStore.adjust()`。另见 G18 修复建议中的"与 S9 的关系"说明。
...
响应 v5 审查报告（a_v5_iteration_requirement.md）的反馈，本次修订针对全部7个质量问题（1个严重 + 3个中等 + 3个低等）进行逐项处理：

| 质询意见 | 回应 |
|---------|------|
| **1. S5b 对 chatStore 和 SSE 机制的就绪状态未经实际代码验证**（严重，本轮新发现） | 已实际读取 `src/stores/chatStore.ts`（13行）和 `src/types/sse.ts`（30行）全文，确认 chatStore 为骨架实现——`conversations` 为空 `ref([])`，`abortActiveConnection()` 为空函数体，文件内无任何 `EventSource`/`SSE`/`WebSocket` 代码，`SSEMessageEvent`/`SSEErrorEvent`/`ChatMessage` 等类型完全未被引用。基于此验证结果将 S5b 拆分为两个子任务：**S5b-1**（chatStore SSE 核心实现，复杂度高，P0 优先级）——需从头构建 EventSource 连接管理、消息收发、conversation_id 管理、断线重连、多医生会话路由；**S5b-2**（DoctorChatView.vue 组件 + 路由注册，复杂度中高，P1 优先级）——硬依赖 S5b-1。S5b-1 标注为 S5b-2 的硬前置依赖。修改位置：第2节 S5b 条目（全文重写约80行）、8.2 节优先级表（S5b 行替换为 S5b-1/S5b-2 独立行并增加 [†E] 脚注）、8.1 节影响面分级（更新 S5b 阻塞级条目）、第5节技术可行性评估（重写 S5b 评估）、第7节根因分析（更新根因2的证据引用）。 |
| **2. G18 修复建议与 S9 的关系未建立交叉引用，方案选择存在隐性不一致**（中，本轮新发现） | 已在 G18 修复建议中新增"与 S9 的关系"说明段落：(a) 明确 S9 为 punchStore 的三个 action（fetchList/loadMore/fetchAnalysis）实现了 requestId 快照模式，G18 需将该模式扩展到 homeStore 和 lifePlanStore；(b) 明确列出需补充保护的4个 action：`homeStore.fetchHomeData()`、`lifePlanStore.fetchCurrent()`、`lifePlanStore.generate()`、`lifePlanStore.adjust()`；(c) 建议修复顺序为 S9 先于 G18（S9 提供已验证的模板）。同时在 8.2 节新增 [†F] 脚注标注 G18↔S9 的交互关系，在优先级表 P2 G18 行增加对应的脚注引用。修改位置：G18 修复建议（方案A 优先段落）、8.2 节脚注区（新增 [†F]）、8.2 节优先级表 P2 G18 行。 |
| **3. P2 优先级层内部缺少执行排序依据**（中，本轮新发现） | 已在 P2 层级内部增加四段排序标注：(a) S4+S11 最先（#1，同文件区域重叠，无外部依赖）；(b) G14 次之（#2，影响面最广——3个文件10个函数，建议在 S9 完成 requestId 快照后执行更安全）；(c) G18 随后（#3，依赖 S9 的 requestId 模板，需将同一模式扩展到 homeStore/lifePlanStore）；(d) S8 最后（#4，建议在 S1/S2 完成后执行以便 clearAuth 调用缓存清理函数）。同时将 S9 优先级从 P2 上调至 P1，并在 S3+S7 行增加"建议在 S9 完成后再执行"注释。修改位置：8.2 节优先级表（整体重排约10行）。 |
| **4. S5a 缺少后端 API 不可用时的降级实现路径**（中，本轮新发现） | 已在 S5a 修复建议中新增"后端 API 不可用时的降级方案"子项：若 `GET /api/articles/:id` 不存在，可先用 `getArticles()` 全量拉取后在客户端 `find(a => a.id === Number(id))` 筛选，包装为 `getArticle(id)` 函数。明确预期返回类型建议（新建 `ArticleDetail` 接口或复用 `Article` 类型），并注明"待后端接口就绪后仅替换函数实现而无需改组件"。同时更新了 S5a 的复杂度标注（从"低"改为"低至中，取决于后端 API 就绪状态"）和代码证据（补充 useHomeApi.ts 无 getArticle(id) 的事实）。修改位置：第2节 S5a 修复建议（新增降级方案子项 + 更新复杂度）、第2节 S5a 代码证据。 |
| **5. S8 BroadcastChannel 跨标签页同步代码示例存在模块级初始化副作用**（低，本轮新发现） | 已将 S8 BroadcastChannel 代码示例从模块级 IIFE `const channel = (() => { try { return new BroadcastChannel(...) } catch { return null } })()` 改为懒初始化模式：`let channel: BroadcastChannel | null = null` + `function getChannel(): BroadcastChannel | null` 按需创建。同时在 `onmessage` 处理器中增加 `if (token && role)` 状态就绪检查，防止 Pinia Store ref 未完全初始化时调用 `syncFromStorage()`。修改位置：S8 修复建议"跨标签页同步（可选增强）"代码块及实现要点。 |
| **6. 修复建议的验证方法缺少 TypeScript 编译检查步骤**（低，本轮新发现） | 已在 G7（新建 useMarkdown.ts）、G8（新建 errorMessage.ts）、G9（合并 DiabetesTypeView 接口定义——同时增加字段一致性检查步骤）、G12（移动 escapeHtml 到 sanitize.ts）、S5a（新建 ArticleDetailView.vue + 路由）、S5b-1（重写 chatStore.ts）、S5b-2（新建 DoctorChatView.vue + 路由）、S10（新建 sanitize.ts 统一函数）共8个条目的验证方法中各增加一条：`执行 vue-tsc --noEmit 确认无新增编译错误`。修改位置：上述各条目的"验证方法"行末。 |
| **7. 第1节汇总表统计数字与详细条目诊断结论标签存在轻微不一致**（低，本轮新发现） | 已完成三项修正：(a) 汇总表"设计对齐"行严重问题从"3"修正为"2"（仅 S8 和 S13 的详细条目标签为"设计对齐"），合计从"4"修正为"3"；(b) G4 条目头部诊断结论标签从"部分确认"修正为"设计对齐"，与汇总表归类一致（G4 的诊断本质为代码符合设计——设计文档在不同位置分别描述了按钮和无限滚动两种模式，双模式均有设计依据）；(c) 汇总表"不成立"行一般问题从"1"修正为"0"（G4 已移至"设计对齐"行），"设计对齐"行一般问题保持"1"（即 G4），同步更新注脚说明 G4 的诊断本质及标签变更原因。同时更新了汇总表注脚以反映 S5b 拆分后的条目分布。修改位置：第1节汇总表行18-20、第1节注脚第22行、G4 条目头部标签第361行。 |

**修订总结**: 本次 v5 修订全面响应了 a_v5_iteration_requirement.md 审查反馈中的全部7个质量问题（1严重/3中/3低）。核心变更为：(a) S5b 基于 chatStore 实际代码验证结果拆分为 S5b-1（SSE 核心，P0）和 S5b-2（组件+路由，P1），解决了此前四轮迭代均未触及的"chatStore 就绪状态未经验证"这一严重缺陷；(b) G18 与 S9 的交叉引用体系建立——明确 S9 为 G18 提供 requestId 快照模板，列出4个待扩展 action；(c) P2 优先级层内部新增4段排序标注，S9 上调至 P1 以解除 S3/S7 的隐式依赖；(d) S5a 新增后端 API 不可用时的 `getArticles()` 降级路径；(e) S8 BroadcastChannel 代码从模块级 IIFE 改为懒初始化 + 状态就绪检查；(f) 8个条目的验证方法增加 `vue-tsc --noEmit` 编译检查步骤；(g) 汇总表"设计对齐"统计修正（3→2 严重）及 G4 标签修正。修改涉及约25处目标位置，新增约3500字内容。

---

## 修订说明（v5 v2）

响应 v5 质询报告（a_v5_challenge_v1.md）的反馈，质询结论为 CHALLENGED，共识别3个问题（1个一般 + 2个轻微），本次修订逐项处理：

| 质询意见 | 回应 |
|---------|------|
| **1. 8.1 节 S13 同时出现在"中危级"和"无需代码修改"两个互斥类别中**（一般） | 已修正：将 S13 从 8.1 节"中危级（功能部分可用但有偏差）"列表中移除，仅保留在"无需代码修改（仅确认/设计文档更新）"列表中。S13 的实际诊断结论为"设计对齐——代码符合设计文档"，优先级表标注为 P4"设计文档更新"（行动类型：无需代码修改），与"中危级"的定义（功能部分可用但有偏差，隐含需要代码修改）存在矛盾。修正后修复者从 8.1 节可直接确定 S13 无需代码修改，无需跳转至详细条目确认。同时复查了 8.1 节与 8.2 节优先级表的全量对应关系，确认无其他跨类别重复项。修改位置：第8.1节中危级列表（第858行）。 |
| **2. S5b-1 条目缺少显式的"诊断结论"标签行，格式与其他 42+ 条目不一致**（轻微） | 已在 S5b-1 条目首部（"现状"行之前）新增 `**诊断结论**: **确认 — chatStore为骨架实现，需从头构建SSE通信层（conversations空数组、abortActiveConnection空函数体、不含任何EventSource/SSE代码）**` 标签行。同时：(a) 为保持子条目格式统一，S5b-2 条目首部也新增了诊断结论标签；(b) 第1节汇总表注脚中 S5b-1 的诊断结论标签从"不成立"同步修正为"确认"——"不成立"在本报告的诊断结论分类体系中定义为"代码实现优于设计规定，无需修改代码"，而 S5b-1 需要从头构建整个 SSE 通信层（需大量代码修改），使用"不成立"标签存在语义混淆。"确认"标签更准确地反映了该条目的诊断定位；(c) 同步修正汇总表统计："不成立"行严重问题从 1 修正为 0（合计 2→1），"确认"行严重问题从 10 修正为 11（合计 36→37），确保13个原始严重问题（S1-S13）的分布为 11 确认 + 2 设计对齐 + 0 不成立 = 13。修改位置：S5b-1 条目（第148行新增标签行）、S5b-2 条目（第160行新增标签行）、第1节汇总表（第17行/第20行/第22行注脚）。 |
| **3. S5a 修复建议未引用项目中已存在的同类实现模式 `getDiabetesType(id)`**（轻微） | 已在 S5a 修复建议的"可复用模式"行末追加：**特别参考** `useHomeApi.ts:67-72` 的 `getDiabetesType(id)` 函数——该函数实现了按ID获取单个资源详情的完整模式（`GET /api/resource/:id` + 路由参数解析 + 错误处理 + 类型标注），可直接作为 `getArticle(id)` 的实现模板，加速正向实现。经实际代码审查确认，`getDiabetesType(id)` 与所需的 `getArticle(id)` 在 API 调用模式、响应解包、参数编码上完全相同，修复者可直接复用该模式的代码结构。修改位置：第2节 S5a 修复建议"可复用模式"行（第130行）。 |

**修订总结**: 本次 v5 v2 修订全面响应了 a_v5_challenge_v1.md 质询反馈中的全部3个问题（1一般/2轻微）。核心变更：(a) 影响面分级的互斥分类矛盾修复——S13 从"中危级"移除，8.1/8.2 全量一致性复查通过；(b) 结构化字段补齐——S5b-1/S5b-2 新增诊断结论标签，汇总表统计同步修正（不成立 1→0 严重，确认 10→11 严重），注脚一致性对齐；(c) 实现模式补充——S5a 修复建议引用 `getDiabetesType(id)` 作为 `getArticle(id)` 的实现模板。修改涉及约8处目标位置。

---

## 修订说明（v5 v3）

响应 v5 质询报告（a_v5_challenge_v2.md）的反馈，质询结论为 CHALLENGED，共识别1个问题（1个一般），本次修订逐项处理：

| 质询意见 | 回应 |
|---------|------|
| **1. 第1节汇总表"确认"行一般问题统计数字为26，逐项核查实际为27，合计37应为38**（一般） | 已修正：汇总表"确认"行一般问题从26修正为27，合计从37修正为38（11+27）。经逐项核对全部29个G条目的头部诊断结论标签：G1/G2/G3/G6/G7/G8/G9/G10/G11/G12/G13/G14/G15/G16/G17/G18/G19/G20/G21/G22/G23/G24/G25/G26/G27/G28/G29共27项标注为"确认"，G4标注为"设计对齐"，G5标注为"不成立"。v5 v2修订说明声称已修正汇总表统计问题（响应原v5问题7），但该计数偏差未被检出——提示汇总表的统计数字缺少与详细条目诊断结论标签的自动化逐项核对机制。建议在修订流程中增加统计数字自动校验步骤：每轮修订后遍历所有条目的头部诊断结论标签并重新计数，与汇总表对比。修改位置：第1节汇总表第17行。 |

**修订总结**: 本次 v5 v3 修订响应了 a_v5_challenge_v2.md 质询反馈中的1个一般问题。核心变更：汇总表统计数字修正（确认行一般26→27，合计37→38），经逐项核实全部29个G条目的诊断结论标签确认。修改涉及1处目标位置。

---

## 修订说明（v5 v4）

响应 v5 质询报告（a_v5_challenge_v3.md）的反馈，质询结论为 CHALLENGED，共识别1个问题（1个一般），本次修订逐项处理：

| 质询意见 | 回应 |
|---------|------|
| **1. S8 在 8.1 节"影响面分级"中完全缺失**（一般） | 已修正：将 S8（Token 明文存储安全隐患，严重程度"中"）补充至 8.1 节"高危级（影响核心体验但功能可降级运行）"列表中。S8 涉及 XSS 窃取风险这一安全维度，其影响超越了"功能部分可用但有偏差"的范畴（中危级定义），安全风险客观存在且影响用户会话安全性这一核心体验。当前 localStorage 方案功能正常，仅安全性不足，符合"功能可降级运行"标准。同时复查了 8.1 节与 8.2 节优先级表的全量对应关系：8.1 节原覆盖 3（阻塞级）+ 3（高危级）+ 10（中危级）+ 23（低危级）+ 4（无需代码修改）= 43 项，报告共涉及 44 个编号条目（S5 拆分为 S5a/S5b-1/S5b-2 后共 15 个 S 条目 + 29 个 G 条目），缺失的 1 项即为 S8。修正后 8.1 节高危级从 3 项增至 4 项（S1/S2/S6/S8），总计 44 项全覆盖。经逐项比对 8.2 节优先级表，确认除 S8 外无其他遗漏项。修改位置：第 8.1 节高危级列表（第852-855行区域）。 |

**修订总结**: 本次 v5 v4 修订响应了 a_v5_challenge_v3.md 质询反馈中的1个一般问题。核心变更：S8 补充至 8.1 节高危级影响面分级，8.1 节与 8.2 节全量对应关系复查通过（44 项全覆盖）。修改涉及 1 处目标位置。