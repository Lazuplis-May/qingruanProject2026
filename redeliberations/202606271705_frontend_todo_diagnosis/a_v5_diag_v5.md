...
> **诊断日期**: 2026-06-27
> **版本**: v6（修订版，响应 v5 质询反馈——含 S5b-2 Consultation.vue 占位状态验证与修复范围修正、根因2 因果链完善、S5b-2 复杂度上调等 3 项修订）
...
t  - **后端 API 不可用时的降级方案（v5新增）**: 若 `GET /api/articles/:id` 未就绪，可用 `getArticles()` 全量拉取 + `find(a => a.id === id)` 筛选包装为 `getArticle(id)`。**但降级路径存在关键限制**——`Article` 类型（api.ts:124-139）不含 `content`/`body` 字段（仅 `summary`），降级交付物仅为"文章元数据详情页"无法渲染正文。模板中可通过 `v-if="article.content"` 区分两态。后端接口就绪后正文自动激活。复杂度重评估：接口就绪时"低"，未就绪时"中"。建议返回类型新建 `ArticleDetail extends Article { content: string }`。
...
- **关键逻辑**: (a) 实现 EventSource 连接管理（`new EventSource(url)` + 断线重连指数退避）；(b) 消息收发——`sendMessage(doctorId, content)` 通过 REST API 发送用户消息，SSE 流式接收 AI 回复并增量追加到 `conversations` 数组；(c) `conversation_id` 管理——首次对话创建 conversation，后续追加使用同一 conversation_id，跨会话持久化于 localStorage；(d) 实现 `abortActiveConnection()` 函数体——关闭当前 EventSource 连接；(e) 多医生会话路由——不同 doctorId 维护独立 conversation_id 映射（`Map<number, string>`）；(f) `fabOpen` 状态管理——控制 Consultation 悬浮按钮展开/收起；(g) 消息流式渲染——AI 回复 chunk 增量追加，ref 驱动 UI 更新。
- **参考类型**: `src/types/sse.ts` — `SSEMessageEvent`（event/message）、`SSEMessageEndEvent`（event/message_end）、`SSEErrorEvent`（event/error）、`ChatMessage`（id/role/content/timestamp）。
- **边界条件**: SSE 连接中断时自动重连（指数退避，max 5次）；浏览器不支持 EventSource 时降级为轮询或展示提示；多标签页仅最新标签页维持 SSE 连接。
...
- **修复建议**:
  - **修改文件**: `src/composables/useApi.ts` — 在 `api` 实例的请求拦截器中集成 cancel token → 各 API composable 支持传入 `signal` 参数 → `src/views/Home.vue`、`src/views/LifePlan.vue`、`src/views/Punch.vue` 的 `onUnmounted` 中调用 `abort()`
	  ...
	  - **与 S9 的关系（v5新增）**: S9（`fetchAnalysis` 竞态保护）为 punchStore 的 `fetchList`/`loadMore`/`fetchAnalysis` 三个 action 实现了 `requestId` 快照模式。G18 需将该模式扩展到 homeStore 和 lifePlanStore。具体需补充保护的 action：(1) `homeStore.fetchHomeData()` —— `Promise.allSettled` 并行请求需增加 page instance token；(2) `lifePlanStore.fetchCurrent()` —— 增加 requestId 快照；(3) `lifePlanStore.generate()` —— 增加 requestId 快照（快速重复点击场景）；(4) `lifePlanStore.adjust()` —— 增加 requestId 快照。S9 完成后 punchStore 已完备，G18 仅需补充 homeStore 和 lifePlanStore。在 8.2 节优先级表中 S9 行和 G18 行通过 [†A] 脚注互相引用。
...
- **S5a（ArticleDetailView组件+路由）**: 需新建一个 Vue 组件文件并注册路由。涉及 marked+DOMPurify 渲染，与 LifePlan 内容渲染模式一致。复杂度评估（v5修正）——后端 `GET /api/articles/:id` 接口就绪时复杂度'低'；未就绪时降级方案复杂度'中'。**降级路径限制**：`Article` 类型（api.ts:124-139）不含 `content`/`body` 字段，使用 `getArticles()` + 客户端筛选的降级方案仅能交付"文章元数据详情页"（标题/作者/摘要/阅读量），无法渲染文章正文。降级模式下模板通过 `v-if="article.content"` 区分两态。预期返回类型建议新建 `ArticleDetail extends Article { content: string }`。
- **S5b-1（chatStore SSE核心实现）**: **v5 经实际代码验证**——`chatStore.ts`（13行）为骨架实现：`conversations = ref([])` 空数组，`abortActiveConnection()` 空函数体，无任何 EventSource/SSE/WebSocket 代码。`sse.ts` 中定义的类型完全未被引用。需从头构建：EventSource连接管理、消息收发、断线重连（指数退避）、conversation_id 管理（localStorage 持久化）、多医生会话路由（Map<number, string>）、fabOpen 状态管理。**复杂度：高**（涉及 SSE 协议实现、断线重连、消息流式渲染）。硬前置依赖——S5b-2 必须在 S5b-1 完成后才能进行。
...
|------|---------------|---------------|
| 状态管理未对齐 | S1, S2 | — |
| 路由拆分未跟进 | S5a, S5b（含 Consultation.vue 占位——医生列表入口页未实现，属于 S5b 修复范围中的独立前置条件，参见 S5b-2 修复建议 v6 修订） | S6（前置依赖 S5a） |
| 筛选联动遗漏 | S3, S7 | — |
| 跨模块接收不完整 | S4, S11 | — |
...
|:------:|------|:-----:|:------:|---------|-----------|------|
| **P0** | S5a | 高 | 代码修复 | 待确认：后端 GET /api/articles/:id 接口是否存在 [†D] | — | ArticleDetailView.vue + 路由，需确认后端 API 就绪状态 |
| **P0** | S5b-1 | 高 | 代码修复 | 无（chatStore 骨架需从头构建 SSE） | — | 实现 chatStore SSE 核心（EventSource连接/消息收发/重连/conversation_id管理），硬前置依赖 |
| **P1** | S5b-2 | 高 | 代码修复 | **→ S5b-1**；Consultation.vue 医生列表 UI（独立前置——当前为7行占位页面，需完整构建医生卡片列表及跳转入口。参见根因2 v6修订和 S5b-2 修复建议。若剥离为独立 Todo 项则标注为 P0 前置任务） | — | DoctorChatView.vue 组件 + 路由注册 + Consultation.vue 医生列表重构（v6修正：S5b-2 实际工作量包含入口页构建，复杂度从"中高"上调至"高"） |
| **P1** | S6 | 中 | 代码修复 | **→ S5a** | — | 文章跳转修复依赖 ArticleDetailView 路由存在 |
| **P1** | S1, S2 | 高 | 代码修复 | 无 | **批处理 S1+S2** | 同为 sessionStorage 缓存模式，用同一模板实现 [†B] |
| **P1** | S3, S7 | 中 | 代码修复 | S9（竞态保护——建议在 S9 完成后再执行 S3+S7） | **批处理 S3+S7** | 同在 Punch.vue + punchStore.ts，修改区域重叠；S7 修复建议明确要求 fetchAnalysis 需先有竞态保护 |
| **P2** | S9 | 中 | 代码修复 | 无 | — | [†A] 竞态保护，建议上调至 P1（作为 S3/S7 批处理组的前置项），或 S3/S7 在 S9 完成后再执行 |
| **P2** | G14 | 中 | 代码修复 | S9（建议） | — | [†A][†C] API 响应拦截器统一处理 success 字段检查，影响面最广 |
| **P2** | S4, S11 | 中/低 | 代码修复 | 无 | **批处理 S4+S11** | 同在 LifePlan.vue onMounted + computed，修改区域重叠 |
| **P2** | S8 | 中 | 代码修复 | S1/S2（建议） | — | [†B] sessionStorage 迁移，涉及 authStore + 路由守卫联动 |

> **P2 层内部执行排序（v5新增）**: S9 建议最先（解除 S3/S7 P1 批处理组对 fetchAnalysis 竞态保护的隐式依赖，S7 修复建议明确要求 fetchAnalysis 需先有竞态保护）；G14 次之（影响全部 10 个 API 函数的错误处理路径，影响面最广）；S4+S11 随后（同文件区域重叠，批处理）；S8 最后（建议 S1/S2 完成后执行，共享 sessionStorage 清理机制）。
...
> - **[†D]**: S5a 依赖后端 GET /api/articles/:id 接口。当前 useHomeApi.ts 仅含 getArticles（列表），不存在 getArticle(id) 单篇详情函数。修复前需确认后端接口是否已就绪，若未就绪需新增 API 函数及对应后端路由。
