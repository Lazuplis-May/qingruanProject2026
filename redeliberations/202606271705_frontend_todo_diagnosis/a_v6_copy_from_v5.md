...
> **诊断日期**: 2026-06-27
> **版本**: v8（修订版，响应组件B质量审查反馈——含 G18 pageInstanceId 变量声明位置补充、"阻塞"术语双义消除、G4 行动类型标注一致性修正、集成验证策略新增、S5a 直接API验证路径建议、8.2节预估工时列新增等 6 项修订）
...
- **设计意图与类型约束的张力分析（v7新增，v8修订）**: 设计文档 1.6.1 节路由映射表要求 `/news/article/:id → ArticleDetailView.vue`，暗示文章详情页应包含完整正文内容。但 `Article` 类型（api.ts:124-139）不含 `content`/`body` 字段且注释标注"列表项"。此张力存在两种可能：(a) 设计文档预期了独立的文章详情 API（`GET /api/articles/:id` 返回含 `content` 的扩展类型，如 `ArticleDetail extends Article { content: string }`），但类型定义阶段未创建 `ArticleDetail`——此时修复工作需前后端联调（后端新增接口 + 前端新增类型和组件）；(b) 设计文档撰写时未考虑类型约束，文章详情页的设计意图仅展示已有多字段（标题/作者/摘要/阅读量等元信息）而不含正文——此时 S5a 为纯前端任务。诊断无法从现有文档中确定答案，修复者实施前需与设计方/后端确认。**操作建议（v8新增）**: 在与设计方或后端沟通之前，修复者可先通过以下方式直接验证 `GET /api/articles/:id` 的实际返回数据，这比口头确认更快速、更准确：(1) 打开浏览器 DevTools Network 面板，访问已部署的后端环境中的 `/api/articles/1`（或任意已知文章 ID），直接观察响应体 JSON 是否包含 `content`/`body` 字段；(2) 或使用 `curl -H "Authorization: Bearer <token>" https://<api-host>/api/articles/1` 命令行直接请求 API。若响应体含正文内容字段，则方案 (a) 成立，需新建 `ArticleDetail` 类型；若仅含列表字段，则方案 (b) 成立，降级方案以元数据详情页交付。
...
- **S5a（ArticleDetailView组件+路由）**: 需新建一个 Vue 组件文件并注册路由。涉及 marked+DOMPurify 渲染，与 LifePlan 内容渲染模式一致。复杂度评估（v7修订）——后端 `GET /api/articles/:id` 接口就绪时复杂度'低'；未就绪时降级方案复杂度'中'（且降级方案需先验证 `getArticles()` 无参调用的后端分页默认行为——当前确认为分页 API，后端在缺失 `page`/`pageSize` 时可能仅返回首页，导致 `find()` 筛选失效）。**降级路径限制**：`Article` 类型（api.ts:124-139）不含 `content`/`body` 字段，使用 `getArticles()` + 客户端筛选的降级方案仅能交付"文章元数据详情页"（标题/作者/摘要/阅读量），无法渲染文章正文。降级模式下模板通过 `v-if="article.content"` 区分两态。预期返回类型建议新建 `ArticleDetail extends Article { content: string }`。**设计意图待确认（v7新增）**：设计文档 1.6.1 节路由映射暗示详情页含正文，但 Article 类型不含 content，需与设计方/后端确认是否预期了独立文章详情 API。
- **S5b-1（chatStore SSE核心实现）**: **v5 经实际代码验证，v7 经设计文档验证**——`chatStore.ts`（13行）为骨架实现：`conversations = ref([])` 空数组，`abortActiveConnection()` 空函数体，无任何 SSE/fetch/WebSocket 代码。`sse.ts` 中定义的类型完全未被引用。需从头构建：**fetch + ReadableStream SSE 连接管理**（v7 修正：项目设计文档 `docs/1_requirements_analysis_v1.md` 第15.2节已将 EventSource API 标记为严重技术错误并修正为 `fetch + ReadableStream`——EventSource 不支持自定义 HTTP 头，无法携带 JWT Token 通过认证；`docs/1_requirements_analysis_v2.md` 第989行、`docs/2_detailed_design_v3.md` 第2373行均明确 `fetch POST` + `ReadableStream` 按 `\n\n` 分隔事件块为唯一 SSE 消费方案）、消息收发（`sendMessage` REST API 发送 + SSE 流式接收）、断线重连（fetch 重试 + 指数退避，替代 v6 的 EventSource 自动重连语义）、conversation_id 管理（localStorage 按 doctorId 区分存储键）、多医生会话路由（Map<number, string>）、fabOpen 状态管理、AbortController 集成（与 G18 协同）。**复杂度：高**（涉及 SSE 协议在认证场景下的 fetch+ReadableStream 实现、chunk→文本→事件块→JSON 解析管道、断线重连、消息流式渲染）。硬前置依赖——S5b-2 必须在 S5b-1 完成后才能进行。
...
|:------:|------|:-----:|:------:|---------|:------:|-----------|------|
| **P0** | S5a | 高 | 代码修复 | 待确认：后端 GET /api/articles/:id 接口是否存在 [†D] | 4-8h（取决于后端API就绪状态——接口就绪2-4h，需降级方案4-8h） | — | ArticleDetailView.vue + 路由，需确认后端 API 就绪状态 |
| **P0** | S5b-1 | 高 | 代码修复 | 无（chatStore 骨架需从头构建 fetch+ReadableStream SSE） | 16-24h（2-3人天——含fetch+ReadableStream SSE连接管理、chunk→文本→事件块→JSON解析管道、断线重连指数退避、conversation_id管理、多医生会话路由、AbortController集成、消息流式渲染） | — | 实现 chatStore SSE 核心（fetch+ReadableStream SSE连接/消息收发/重连/conversation_id管理），硬前置依赖 |
...
> **预估工时汇总（v8新增）**: 上述预估为粗略数量级参考（基于单人开发估算），不包含代码审查、联调、后端依赖等待时间。不确定性高的条目已在工时列中以条件形式标注（如"取决于后端API就绪状态"）。**P0+P1 总计约 43-70h（5.4-8.8人天）**，其中 S5b-1 和 S5b-2 合计 32-48h 为最大单项投入；P2 总计约 7-11h；P3 总计约 7-11h；P4 总计约 7-12h。全部42项问题修复**总预估工时约 64-104h（8-13人天）**，实际工时会因开发者熟悉度、后端接口就绪状态、代码审查轮次等因素浮动。
>
...
| **P4** | S12 | 低 | 仅确认 | 无 | 0（无需改动，可选验证后端写入延迟） | — | 间接一致性模型正确，无需代码修改，可选文档补充 |
...
| 序号 | 场景 | 验证点 |
|:----:|------|--------|
| 1 | 标签页A登录 → 新开标签页B访问任意页面 | 标签页B因无 token 应重定向登录页（sessionStorage 隔离预期行为）。若已实现 BroadcastChannel 可选增强，标签页B应自动同步认证状态 |
| 2 | 标签页A登出 → 标签页B刷新 | 标签页B应同步登出（若已实现 BroadcastChannel 可选增强） |
