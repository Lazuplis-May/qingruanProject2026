...
- **修复建议（短期过渡）**:
  ...
  - **跨标签页同步（可选增强）**: sessionStorage 迁移后，每个新标签页因缺失 token 将强制跳转登录页，形成 UX 退化。以下为 BroadcastChannel 最小实现方案（标注为可选增强，核心修复可独立交付）：
    ```typescript
    // 在 authStore.ts 中（仅当浏览器支持 BroadcastChannel 时激活）
    const channel = (() => {
      try { return new BroadcastChannel('qrzl_auth_sync') } catch { return null }
    })()
    if (channel) {
      channel.onmessage = (e: MessageEvent) => {
        if (e.data?.type === 'AUTH_CHANGED') syncFromStorage()
      }
    }
    // 在 setAuth() / clearAuth() 末尾广播：
    channel?.postMessage({ type: 'AUTH_CHANGED', timestamp: Date.now() })
    ```
