...
- 用户发送消息后 3 秒内开始收到流式回复（计时起点为用户点击发送按钮的时刻，终点为首个 SSE 事件 data 字段到达前端的时刻；基于 DeepSeek API 一般响应时间的推断，实际值可能受 API 负载和网络条件影响）。
...
- 用户完成所有步骤并提交后，10 秒内展示风险等级和对应的预防/管理建议（计时起点为用户点击提交按钮的时刻，终点为风险等级和预防/管理建议内容渲染完成的时刻；10 秒为合理预期而非硬性约束，实际耗时受 Dify 工作流执行时间和 DeepSeek API 响应时间影响）。
...
**验收标准**：
- AI 生成方案至少包含 4 条饮食项（覆盖早/中/晚/加餐）和 3 条运动项（覆盖晨间/晚间/周末），方案内容在 15 秒内开始返回（计时起点为用户点击生成方案的时刻，终点为第一条方案项内容渲染完成的时刻；15 秒为合理预期而非硬性约束，实际耗时受 Dify 工作流执行时间和 DeepSeek API 响应时间影响）。
...
**验收标准**：
- 用户选择分类标签后，系统在 15 秒内生成并展示完整的文章（含标题、标签、正文）（计时起点为用户点击分类标签的时刻，终点为文章正文渲染完成的时刻；15 秒为合理预期而非硬性约束，实际耗时受 Dify 工作流执行时间和 DeepSeek API 响应时间影响）。
...
**验收标准**：
- 管理员通过自然语言完成一次完整 CRUD 操作（如"新增一位医生，姓名张三，科室内分泌科"），全流程在 10 秒内完成并返回操作结果（计时起点为管理员发送消息的时刻，终点为操作结果反馈在对话界面中渲染完成的时刻；10 秒为合理预期而非硬性约束，实际耗时受 Dify Agent 推理时间、Text2SQL 执行时间和 Express 响应时间影响）。
...
- **用户风险信息**（user_risk_info）：用户ID、年龄、性别、身高、体重、家族史、腰围、收缩压、妊娠状态、糖尿病病史（diabetes_history，TEXT，与 POST /api/risk/predict 的 diabetes_history 参数对应）、原始输入信息、疾病类型。
- **生活方案**（life_plans）：方案ID、用户ID、类型（饮食/运动/其他）、排序（order，INTEGER，控制显示顺序，与时段映射约定见4.5节）、时间、标题、内容、是否活跃（is_active，BOOLEAN，默认 TRUE，方案调整时标记旧方案为 FALSE，见 4.5 节方案调整策略）。
