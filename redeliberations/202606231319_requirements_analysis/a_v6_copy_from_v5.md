...
SSE 代理转发策略与 6.4 节医师对话接口一致——Express 服务端以流模式透传 Dify Agent 返回的 SSE 事件至前端，前端使用 fetch + ReadableStream 方式消费流数据（不使用 EventSource API，原因同 6.4 节：认证 SSE 端点需在请求头中携带 JWT Token，而 EventSource 不支持自定义请求头）。
