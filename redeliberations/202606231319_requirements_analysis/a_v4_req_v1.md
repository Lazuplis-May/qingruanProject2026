...
**会话管理**：用户进入医师对话界面时默认创建新会话（不传 conversation_id），Dify 返回新会话 ID 后前端保存至 localStorage。前端在 localStorage 中维护"当前活跃对话的 conversation_id"，后续对话消息自动携带该 ID 以保持上下文连续性。历史对话列表通过 Dify 的会话列表 API 获取（建议新增 GET /api/chat/doctor/:id/conversations 端点，Express 代理层转发至 Dify 会话列表接口），用户可从历史会话列表中选择并恢复对话。会话不设置自动过期，由用户在 UI 中手动删除。
...
**管理员鉴权**：管理员接口在 JWT role 校验之外，额外校验请求来源的合理性（如检查 Token 是否在合理的使用时效内）。管理员的数据库操作通过 Express `/api/admin/execute` 接口执行，该接口需同时通过 Token 有效性校验和 role=admin 校验两层鉴权。
...
2. **AI 驱动的 Text2SQL 路径**：管理员的智能管理操作和 AI 助手的部分数据库查询操作通过 Dify Agent 的 Text2SQL 工具间接操作数据库。用户（管理员或 AI 助手对话中的普通用户）以自然语言描述操作意图，Dify Agent 借助 DeepSeek 模型将自然语言转换为 SQL 语句后，调用 Express 的 `/api/admin/execute` 端点执行。此路径适用于需灵活查询、无法预定义接口的场景。AI 助手在对话中为普通用户执行个人信息查询、打卡记录查询、健康建议生成等操作时也可走此路径；但需注意 AI 助手为普通用户操作时权限受限——仅能查询/修改当前用户本人的数据，不得越权操作其他用户数据。此权限约束由 Dify Agent 的系统提示词和 Express 端点的 Token 用户身份校验共同保证。

前端在任何情况下均不直接连接数据库——两条路径均通过 Express 服务接口完成数据库读写。
...
以上初始数据的填充方式推荐使用数据库初始化 SQL 脚本（在 SQLite 数据库文件创建时执行），以确保部署环境的可复现性。具体 SQL 脚本内容和执行时机在概要设计阶段确定。

### 5.1 新增 API 端点对应的数据操作

本节所列数据需求对应的新增 API 端点（糖尿病类型查询、会话列表查询等）的完整接口规格见第 6 节。
...
| GET | `/api/risk/history` | 获取当前用户的历史风险预测记录 | 是 |

### 6.4 医师咨询相关

| 方法 | 端点 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/doctors` | 获取医生列表 | 否（浏览） |
...
| 方法 | 端点 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/punch` | 记录一次打卡（含 plan_id、punch_type、completion_status） | 是 |
| GET | `/api/punch/list` | 获取打卡记录列表（支持日期范围和类型筛选参数） | 是 |
...
| 方法 | 端点 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/articles` | 获取文章列表（支持分类筛选参数） | 否（浏览） |
...
| GET | `/api/articles/collections` | 获取当前用户的收藏文章列表 | 是 |
...
| GET | `/api/assistant/advice` | 获取当前用户的健康建议历史列表（life_advice 表） | 是 |
...
SSE 代理转发策略与 6.4 节医师对话接口一致——Express 服务端以流模式透传 Dify Agent 返回的 SSE 事件至前端。

### 6.10 管理相关

| 方法 | 端点 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/admin/execute` | 执行自然语言转 SQL 的数据库操作（需 role=admin） | 是 + admin |
| GET | `/api/admin/logs` | 获取管理员操作日志列表 | 是 + admin |
...
| 工作流标识（推荐命名） | 类型 | 用途 | 所属功能模块 | 触发端点 | 核心 inputs 变量 |
|----------------------|------|------|------------|---------|-----------------|
| `diabetes-risk-prediction` | 工作流 | 根据用户健康数据计算糖尿病风险评分并生成建议 | 4.4 糖尿病风险预测 | `/api/dify/workflow/:workflow_id` | `age` (number, 必填)、`gender` (string, 必填)、`height` (number, 必填)、`weight` (number, 必填)、`waist` (number, 可选，已在4.4节定义估算公式)、`systolic_bp` (number, 可选，已在4.4节定义估算公式)、`family_history` (string, 必填)、`pregnancy` (boolean, 仅女性，已在4.4节定义)、`diabetes_history` (string, 必填) |
| `life-plan-generator` | 工作流 | 根据用户身体信息和生活习惯生成个性化饮食与运动方案 | 4.5 生活方案 | `/api/dify/workflow/:workflow_id` | `health_info` (object, 必填，含age/gender/height/weight等已在4.5节定义)、`preferences` (object, 必填，含dietary/activity偏好，已在4.5节定义) |
| `health-article-generator` | 工作流 | 根据用户健康信息生成健康资讯分类标签和完整文章 | 4.6 健康资讯 | `/api/dify/workflow/:workflow_id` | `user_id` (string, 必填，用于查询用户健康数据，数据来源优先级已在4.6节定义)、`category` (string, 可选，用户选择的分类标签) |
| `punch-analysis` | 工作流 | 对用户打卡记录进行汇总分析和趋势解读 | 4.7 打卡记录与分析 | `/api/dify/workflow/:workflow_id` | `user_id` (string, 必填)、`date_range` (object, 可选，含start/end日期，已在4.7节定义筛选支持) |
| `diabetes-assistant-agent` | Agent | 通过工具调用完成知识问答、风险预测、方案生成、信息查询等综合操作 | 4.8 AI 智能助手 | `/api/dify/agent/:agent_id` | `query` (string, 必填，用户的自然语言消息，已在6.9节定义) |
| `admin-manager-agent` | Agent | 将管理员的自然语言指令转为SQL操作数据库 | 4.9 智能管理 | `/api/dify/agent/:agent_id` | `query` (string, 必填，管理员的自然语言指令，已在6.10节定义) |
| `doctor-chat-{id}` | 聊天助手 | 以特定科室医生的角色进行一对一健康咨询对话 | 4.2 医师咨询 | `/api/chat/doctor/:id` | `query` (string, 必填，用户的对话消息，已在6.4节定义) |
