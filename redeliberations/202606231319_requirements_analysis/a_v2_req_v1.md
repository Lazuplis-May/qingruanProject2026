...
用户首次使用需注册账号（用户名、密码）。注册时密码通过安全通道传输至服务端，由服务端使用 bcrypt 算法进行哈希存储（具体加密细节见 4.10 节），注册信息通过 Express 常规 CRUD 接口（POST /api/auth/register）写入数据库。已注册用户可登录后使用完整功能。
...
**认证流程**：用户提交用户名和密码后，前端将密码经 bcrypt 算法哈希处理后发送至服务端（或前端发送明文、服务端进行哈希校验——具体方案取决于 Express 接口设计，推荐服务端哈希校验以避免密码哈希在传输中被截获后重放）。服务端查询 users 表验证凭据，验证通过后生成 JWT Token（Payload 含 user_id 和 role 字段），返回给前端。
