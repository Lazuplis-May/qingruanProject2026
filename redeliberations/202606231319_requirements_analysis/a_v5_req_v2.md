...
**管理员鉴权**：管理员接口在 JWT role 校验之外，额外校验请求来源的合理性（如检查 Token 是否在合理的使用时效内）。管理员的数据库操作和 AI 助手的 Text2SQL 操作均通过 Express `/api/admin/execute` 接口执行。该端点采用双认证模式，根据请求来源区分鉴权路径：
...
- **浏览器直连场景**（管理员通过智能管理界面操作）：请求携带用户的 JWT Token（Authorization: Bearer \<jwt\>），Express 从 Token 中提取 user_id 和 role 进行分级鉴权。
...
| 方法 | 端点 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/admin/execute` | 执行自然语言转 SQL 的数据库操作。采用双认证模式——浏览器直连场景通过 JWT Token.role 分级鉴权（admin 可执行任意 SQL，user 仅限操作本人数据）；Dify Agent 回调场景通过系统级 API Key + 请求体 user_id 参数鉴权（仅限操作对应 userId 的本人数据）。双认证机制详见第 5 节 Text2SQL 路径说明 | 是（浏览器直连）/ API Key（Dify 回调） |
