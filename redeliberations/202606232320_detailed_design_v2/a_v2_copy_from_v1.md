...
### 4.2 各页面状态管理方案 (Pinia Store + 本地存储)

> **架构说明**: v2 使用 Pinia Store 管理跨组件共享状态，替代 v1 的 sessionStorage 中转缓存和 postMessage 消息总线。页面私有状态使用组件内 `ref`/`reactive`。

| 页面 | 状态存储位置 | 存储内容 | 生命周期 |
|------|------------|---------|---------|
| App.vue | Pinia authStore | JWT Token, user info, role, mustChangePassword（替代原 localStorage + sessionStorage 中转） | 跨会话 (pinia-plugin-persistedstate) |
| App.vue | Pinia chatStore | AI对话状态 (conversation_id, 消息列表, fabOpen, SSE连接状态) | 页面卸载清除 |
| App.vue + 各组件 | Pinia riskFormStore | 风险预测表单多步骤数据（替代原 sessionStorage transfer_data） | 页面卸载或提交成功后清除 |
