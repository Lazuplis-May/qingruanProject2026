...
└── 公共状态
    ...
    ├── Pinia persist (pinia-plugin-persistedstate): authStore持久化
...
| DOMPurify | 3.x | HTML净化库，marked.js渲染Markdown后防XSS | `/static/lib/purify.min.js` |
...
| DOMPurify | 3.x | HTML净化库，用于marked.js渲染后移除XSS危险标签/属性 | CDN或 `/static/lib/purify.min.js` |
...
| 路径 | 对应组件 (懒加载) | meta.authRequired | meta.adminRequired | 说明 |
|------|------------------|-------------------|--------------------|------|
...
router.beforeEach((to, from, next) => {
  ...
  // 4. 管理员路由额外检查role
  if (to.meta.adminRequired && authStore.role !== 'admin') {
    return next('/home');
  }
...
interface AuthActions {
  login(username: string, password: string): Promise<void>;     // 调用 POST /api/auth/login
  logout(): Promise<void>;                                       // 调用 POST /api/auth/logout
  setToken(token: string): void;                                 // 手动设置Token (从localStorage恢复)
  clearToken(): void;                                             // 清除Token（登出/过期）
...
// stores/chatStore.ts
interface ChatState {
  conversationId: string | null;   // Dify对话会话ID
...
interface RiskFormActions {
  saveStep(step: number, data: Partial<RiskFormData>): void;
  saveResult(result: RiskResult): void;  // 保存后 LifePlan.vue 可读取
  reset(): void;
}
```
...
// stores/chatStore.ts
interface ChatState { conversationId: string | null; messages: ChatMessage[]; fabOpen: boolean; isStreaming: boolean; }
...
> - **sessionStorage**: Vue SPA 中极少使用（页面切换不销毁 Pinia Store，无需 sessionStorage 中转）
...
| Consultation.vue | localStorage | conversation_doctor_{id} (多医生会话ID, 跨会话持久化) | 跨会话 (Vue SPA 中 localStorage 对跨会话持久化仍有效) |
...
| Admin.vue | localStorage | admin_conversation_id (跨会话持久化) | 跨会话 |
