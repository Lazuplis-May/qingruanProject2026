...
│   │   ├── Home.vue              # 系统首页
│   │   ├── Consultation.vue      # 医师咨询 (医生列表/对话)
│   │   ├── LifePlan.vue          # 生活方案
│   │   ├── News.vue              # 健康资讯 (列表/详情/收藏)
│   │   ├── Profile.vue           # 个人中心
│   │   ├── Risk.vue              # 糖尿病风险预测
│   │   ├── Punch.vue             # 打卡记录与分析
...
|-------|------|------|-----------|-------------|
| authStore | stores/authStore.ts | 登录态管理 | token: string\|null, user: User\|null, role: string, mustChangePassword: boolean | login(), logout(), setToken(), clearToken() |
| chatStore | stores/chatStore.ts | AI对话状态 | conversationMap: Map<string, string>, messages: Message[], fabOpen: boolean, isStreaming: boolean | sendMessage(), toggleFab(), navigate(), getConversationId(), setConversationId(), removeConversationId() |
...
| /consultation/doctor/:id | () => import('@/views/Consultation.vue') | true | false | 医师对话(路由参数传医生ID) |
| /life-plan | () => import('@/views/LifePlan.vue') | true | false | 生活方案 |
| /news | () => import('@/views/News.vue') | false | false | 健康资讯列表 |
| /news/article/:id | () => import('@/views/News.vue') | false | false | 文章详情 |
| /profile | () => import('@/views/Profile.vue') | true | false | 个人中心 |
| /profile/risk | () => import('@/views/Risk.vue') | true | false | 风险预测(我的Tab子页面) |
| /profile/punch | () => import('@/views/Punch.vue') | true | false | 打卡记录(我的Tab子页面) |
| /profile/advice | () => import('@/views/HealthAdvice.vue') | true | false | 健康建议(我的Tab子页面) |
...
#### Consultation.vue 流程图:

```mermaid
flowchart TD
    A[页面加载] --> B{解析路由参数<br/>route.params.id}
    B -->|有id| C[切换至对话视图]
    C --> D[从chatStore.conversationMap读取<br/>键名: doctor_{id}]
    ...
    P --> N
    O -->|event=message_end| Q[chatStore.setConversationId<br/>('doctor_{id}', conversation_id)]
...
flowchart TD
    ...
    H --> E
    G -->|校验通过| I[riskFormStore.saveStep()<br/>更新内存 + 写入sessionStorage]
    I --> J[切换至下一步]
    J --> K{用户点击"上一步"?}
