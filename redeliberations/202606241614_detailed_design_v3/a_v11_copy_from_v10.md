...
路径3: AI内容生成持久化路径 (Dify工作流->数据库)
  Vue组件 useApi().post()
    ...
    -> INSERT/UPDATE SQLite数据库
       (user_risk_info / life_plans，自动生成的 articles 仅返回前端不落库)
    -> 返回结果给前端
```
...
**转换职责说明**：
- **写入路径**（前端 → API → Express → 不落库）：前端通过 `POST /api/articles/generate` 触发文章生成时，Express 调用 Dify `health-article-generator` 工作流，工作流输出 `tags: string[]` 数组（与 `summary`、`content` 一并产出），Express `articles.js` 路由处理器解析后直接返回给前端展示，**不再执行** `INSERT INTO articles` 写入操作（避免同质化冗余内容污染公共知识库）。`life_advice` 表由 AI 助手对话触发生成，写入逻辑保持落库。
...
- `health-article-generator`：Express在调用前查询 `user_risk_info` 表最新记录（无记录时传入空对象 `{}`，触发通用科普模式），将结果序列化为 `user_health_data` 对象注入 `inputs`。
...
| user_health_data | object | 是 | {age:45, gender:"男", bmi:25.95, family_history:"有", diabetes_type:"2型"} | Express代理层预查询的用户健康数据(来自user_risk_info最新记录，无记录时传空对象{})。Dify工作流**不自行查询数据库**，直接使用此预查询数据进行分类推荐和文章个性化生成。 |
...
节点说明:
1. 接收预查询用户健康数据: 从inputs.user_health_data读取Express代理层预查询的用户健康信息（user_risk_info最新记录，无记录时传空对象{}），Dify工作流不直接访问SQLite数据库
...
2. **修正 `health-article-generator` 降级数据源逻辑**：针对健康资讯生成依赖的生理指标问题，将 `users` 表的无效降级逻辑替换为“无记录时传入空对象 `{}` 触发通用科普模式”，保证 Dify 工作流参数结构完整可用。
