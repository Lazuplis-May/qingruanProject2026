...
└── 公共状态
    ...
    ├── Pinia persist (pinia-plugin-persistedstate): authStore持久化, chatStore.conversationMap持久化
    └── 响应式变量: 当前活跃Tab, FAB弹窗状态
```

**Vue3 SPA 跨组件通信机制**：

1. **登录态同步**: 登录成功 -> authStore.setToken(token) -> Pinia响应式自动通知所有订阅组件更新登录态。
...
4. **跨浏览器标签页的登录态同步**: 在 App.vue 挂载时注册 storage 事件监听。当 localStorage 中 token 或 role 发生变化（如在其他标签页退出登录或切换账号时），如果新值为空，调用 authStore.clearAuth() 并在当前标签页提示并重定向至登录页；如果新值不一致，则更新当前标签页的 authStore.setToken() 以保持多标签页之间的登录态同步。
...
import { setupAxiosInterceptors } from './composables/useApi';
...
|-------|------|------|-----------|-------------|
| authStore | stores/authStore.ts | 登录态管理 | token: string\|null, user: User\|null, role: 'user'\|'admin'\|null, mustChangePassword: boolean | login(), logout(), setToken(), clearToken(), clearAuth() |
| chatStore | stores/chatStore.ts | AI对话状态 | doctorConversations: Map<number, string>, assistantConversationId: string\|null, adminConversationId: string\|null, messages: Message[], fabOpen: boolean, isStreaming: boolean, activeAbortController: AbortController\|null | sendMessage(), toggleFab(), navigate(), getDoctorConversationId(), setDoctorConversationId(), getAssistantConversationId(), setAssistantConversationId(), getAdminConversationId(), setAdminConversationId(), registerAbortController(), abortActiveConnection() |
| riskFormStore | stores/riskFormStore.ts | 风险预测表单数据 | currentStep: number, formData: RiskFormData, result: RiskResult\|null | saveStep(), saveResult(), reset(), loadFromStorage() |
...
const router = createRouter({
  ...
  routes: const routes: RouteRecordRaw[] = [
  {
    path: '/home',
...
为了解决数据库 CHECK 约束中英文小写枚举值与前端页面/API 接口中文展示之间的偏差，本系统采用在 Express 后端设计专用转换模块（`server/utils/mapper.js`）的方案。该转换层具有如下职责和机制：

#### 1.8.1 转换映射字典定义

```javascript
// server/utils/mapper.js
const MAPPINGS = {
  gender: {
    '男': 'male', '女': 'female',
    'male': '男', 'female': '女'
  ...
  diabetes_history: {
    '健康': 'healthy', '糖尿病前期': 'prediabetes', '已确诊': 'diagnosed',
    'healthy': '健康', 'prediabetes': '糖尿病前期', 'diagnosed': '已确诊'
  },
  diabetes_type: {
    '1型': 'type1', '2型': 'type2', '妊娠期': 'gestational', '其他': 'other',
    '1型糖尿病': 'type1', '2型糖尿病': 'type2', '妊娠期糖尿病': 'gestational', '其他特殊类型': 'other',
    'type1': '1型糖尿病', 'type2': '2型糖尿病', 'gestational': '妊娠期糖尿病', 'other': '其他特殊类型糖尿病'
  },
  plan_type: {
    '饮食': 'diet', '运动': 'exercise', '其他': 'other',
    'diet': '饮食', 'exercise': '运动', 'other': '其他'
  },
  completion_status: {
    '已完成': 'completed', '未完成': 'uncompleted',
...
#### 1.8.2 双向转换函数设计
- **输入转换 (`toEnglish`)**：前端请求传入的参数如果包含中文枚举值，控制器在参数校验通过后、调用数据库插入/修改操作前，通过 `toEnglish(category, val)` 将其拦截转换为英文小写值进行落库，从而保证符合 SQLite CHECK 约束。
- **输出转换 (`toChinese`)**：从数据库中读取的记录，在返回给前端响应前，通过 `toChinese(category, val)` 将英文小写枚举值映射还原为中文，确保 API 返回与前端 UI 展示一致。

#### 1.8.3 控制器拦截策略
- 在 `server/routes/risk.js` 的 `POST /api/risk/predict` 端点中：拦截请求体，将 `gender`（'男'/'女' ↔ 'male'/'female'）、`family_history`（'有'/'无' ↔ 'yes'/'no'）、`diabetes_history`（'健康'/'糖尿病前期'/'已确诊' ↔ 'healthy'/'prediabetes'/'diagnosed'）及 `diabetes_type` 进行 `toEnglish` 转换，再写入 `user_risk_info` 表。
- 在 `server/routes/plan.js` 的生成与当前方案端点中：将 `plan_type` 在落库前转换为 `'diet'/'exercise'/'other'`，在 `GET /api/plan/current` 响应返回前通过 `toChinese` 转换回 `'饮食'/'运动'/'其他'`。
- 在 `server/routes/punch.js` 的打卡端点中：落库前将 `punch_type`（'diet'/'exercise'）及 `completion_status`（'completed'/'uncompleted'）转换为英文，返回列表及分析响应前转换回中文。
...
| user_id | INTEGER | NOT NULL, FK | - | 用户ID |
| plan_type | TEXT | NOT NULL, CHECK(plan_type IN ('diet', 'exercise', 'other')) | - | 方案类型：diet=饮食, exercise=运动, other=其他。注：当前版本仅使用'diet'和'exercise'两种类型，'other'为预留扩展值，当前版本前端不渲染此类型的方案项UI |
...
| user_id | INTEGER | NOT NULL, FK | - | 用户ID |
| plan_id | INTEGER | FK, ON DELETE SET NULL | NULL | 关联 Rar 的方案项ID, 支撑依从性分析 |
| punch_time | TEXT | NOT NULL | datetime('now') | 打卡时间 |
| punch_type | TEXT | NOT NULL, CHECK(punch_type IN ('diet', 'exercise')) | - | 打卡类型：diet=饮食, exercise=运动 |
...
```json
{
  "diabetes_history": "string (必填, 健康/糖尿病前期/已确诊 — 将由后端Mapper转换为英文落库)",
  "diabetes_type": "string (可选, 已确诊时填写: 1型/2型/妊娠期/其他特殊类型 — 将由后端Mapper转换为英文落库)",
  "age": 45,
  "gender": "string (必填, 男/女 — 将由后端Mapper转换为英文落库)",
  "height": 170.0,
  "weight": 75.0,
  "waist": "number (可选, 若填写则不能为0，输入0将触发400校验错误)",
  "systolic_bp": "number (可选, 若填写则不能为0，输入0将触发400校验错误)",
  "family_history": "string (必填, 有/无 — 将由后端Mapper转换为英文落库)",
  "pregnancy": false
}
```
...
**SQLite 查询设计**:
```sql
SELECT * FROM life_plans WHERE user_id = ? AND is_active = 1 ORDER BY plan_type, order_num;
```
...
// stores/chatStore.ts
interface ChatState {
  conversationMap: Map<string, string>;  // 按对话场景管理conversation_id: 'doctor_1'→'abc123', 'assistant'→'xyz789', 'admin'→'adm001'
...
interface ChatActions {
  ...
  clearChat(): void;
  getConversationId(key: string): string | undefined;  // 获取指定场景的conversation_id
  setConversationId(key: string, id: string): void;     // 设置指定场景的conversation_id
  removeConversationId(key: string): void;              // 清除指定场景的conversation_id
}
```

**conversation_id 管理策略**: 所有对话场景的 conversation_id 统一由 chatStore 的 `conversationMap` 管理，替代散落在各组件中的 localStorage 分散键名方案。医师对话使用 `doctor_{id}` 作为 key（如 `doctor_1`），AI助手使用 `assistant` 作为 key，管理员对话使用 `admin` 作为 key。chatStore 通过 `pinia-plugin-persistedstate` 将 `conversationMap` 持久化到 localStorage，跨会话恢复时自动还原。各组件通过 `chatStore.getConversationId(key)` 获取对应场景的 conversation_id，通过 `chatStore.setConversationId(key, id)` 更新（如 SSE message_end 事件回调中保存），不再使用 localStorage 直接读写。这解决了 Consultation.vue 使用 `conversation_doctor_{id}`、Admin.vue 使用 `admin_conversation_id` 的存储键名不一致问题。
...
| TOKEN_EXPIRED (过期通知) | Axios 响应拦截器 401 → `authStore.clearToken()` + Toast |
...
interface RiskPredictRequest {
  diabetes_history: '健康' | '糖尿病前期' | '已确诊';
  diabetes_type?: '1型' | '2型' | '妊娠期' | '其他特殊类型';
  age: number;
  gender: '男' | '女';
  height: number;       // cm
  weight: number;       // kg
...
```typescript
// stores/authStore.ts
interface AuthState { token: string | null; user: User | null; role: string | null; mustChangePassword: boolean; }

// stores/chatStore.ts
interface ChatState { conversationMap: Map<string, string>; messages: ChatMessage[]; fabOpen: boolean; isStreaming: boolean; }
...
| Consultation.vue | Pinia chatStore | conversationMap[`doctor_{id}`] (多医生会话ID，由 chatStore 统一管理) | 跨会话 (pinia-plugin-persistedstate 持久化) |
...
| Admin.vue | Pinia chatStore | conversationMap[`admin`] (管理员对话ID，由 chatStore 统一管理) | 跨会话 (pinia-plugin-persistedstate 持久化) |
...
flowchart TD
    ...
    B --> C[调用 chatStore.getDoctorConversationId 获取该医生的会话ID]
    ...
    J --> K[流式读取 ReadableStream 响应块]
    K --> L{解析SSE事件类型?}
    L -->|event=message| M[追加AI消息字符到当前最新气泡中，流式渲染]
    M --> K
    L -->|event=message_end| N[调用 chatStore.setDoctorConversationId 保存会话ID]
    N --> O[关闭流连接]
...
flowchart TD
    ...
    O --> P[用户选定目标分类并确认]
    P --> Q[展示 AI 写作生成中加载状态]
    Q --> R[发送请求 POST /api/articles/generate 携带选定 category]
    R --> S{生成成功?}
...
flowchart TD
    ...
    G --> H[渲染正文并展示固定医学免责声明]
    C -->|否| I[展示404资源不存在或加载失败重试组件]
    
    H --> J[并行请求 GET /api/articles/collections 判断收藏状态]
    J --> K{当前文章是否已被收藏?}
    K -->|是| L[心形收藏图标高亮填充态]
    K -->|否| M[心形收藏图标灰色空心态]
    
    N[用户点击收藏/取消收藏按钮] --> O{当前状态?}
    O -->|已收藏| P[调用 DELETE /api/articles/:id/collect 取消收藏]
    P --> M
    O -->|未收藏| Q[调用 POST /api/articles/:id/collect 添加收藏]
    Q --> L
    
    R[用户点击返回按钮] --> S[router.push('/news') 返回资讯列表页]
```
...
flowchart TD
    ...
    V -->|是| W[authStore.clearToken()<br/>Pinia 响应式自动通知所有组件<br/>展示非阻断 Toast<br/>提供重新登录按钮<br/>router.push /login]
    V -->|否| X[正常返回响应]
```
...
flowchart TD
    ...
    W --> X{用户确认?}
    X -->|是| Y[authStore.logout()<br/>清除 Token + user info<br/>Pinia 响应式自动通知所有组件]
    Y --> Z[router.push /home]
    X -->|否| H
```
...
flowchart TD
    ...
    C --> D[从chatStore.conversationMap读取<br/>键名: admin]
    ...
    O --> M
    N -->|event=message_end| P[chatStore.setConversationId<br/>('admin', conversation_id)]
...
{
  "risk_score": 28,
  "risk_level": "high",  // mapped from Chinese '高风险'
  "risk_level_detail": "根据中国2型糖尿病防治指南评分体系...",
  "diabetes_type": "type2",  // mapped from Chinese '2型'
...
| punch_records | array | 是 | [{plan_id:1, punch_type:"diet", completion_status:"completed", punch_time:"2026-06-23T07:30:00", remarks:"..."}, ...] | Express代理层预查询的打卡记录数组(来自punch_in表, 按user_id和可选date_range筛选)。Dify工作流**不自行查询数据库**，直接使用此预查询数据进行统计计算和分析。 |
...
## 技能7: 生活状态分析
当用户想了解自己的整体健康状态和方案执行情况时，通过execute_SQL工具查询punch_in表进行打卡数据分析（按user_id筛选，按日期范围聚合统计完成率），以友好的表格或列表形式展示分析结果。
...
- 工具名称: execute_SQL
- 工具类型: HTTP API回调
- 回调URL: `POST {EXPRESS_PUBLIC_URL}/api/admin/execute`（EXPRESS_PUBLIC_URL 为环境变量，示例值 `https://your-domain.com`；实训/开发环境可用 `http://localhost:3000`，生产环境必须使用 HTTPS 及固定域名）
- 回调请求体模板:
```json
{
  "sql": "{{sql}}",
...
1. Express 端点接收请求 → 调用 `difyService.callAgentStreaming(apiKey, query, conversationId, user, inputs)`
2. `callAgentStreaming` 内部：`POST {DIFY_API_BASE_URL}/chat-messages`，携带 `response_mode: 'streaming'`
...
| 功能模块 | 验收项 | 量化标准 | 验证方式 |
|---------|--------|---------|---------|
| 认证 (auth) | 注册成功 | POST /api/auth/register 返回 201，含 user_id 和 username | curl + 数据库验证 |
...
| 风险预测 (risk) | 提交预测 | POST /api/risk/predict 返回 risk_score、risk_level、diabetes_type、suggestions | curl + 数据库验证记录写入 |
...
| Consultation.vue | 对话-conversation_id 持久化 | 刷新页面后可从 chatStore.conversationMap 恢复会话，继续对话 | 刷新后发送消息 |
...
| conversation_id 保存 | message_end 事件后 conversation_id 写入 chatStore.conversationMap | DevTools Pinia 检查 |
...
- **第5.2节 Dify应用设计**：更新变量示例值和描述对齐英文小写枚举，更新 `diabetes-risk-prediction` 输出结构示意。
