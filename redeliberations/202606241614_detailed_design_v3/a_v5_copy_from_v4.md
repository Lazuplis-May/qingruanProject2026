...
const ENUM_LABELS = {
  ...
  punch_type: { diet: '饮食', exercise: '运动', other: '其他' },
...
| 层 | gender | family_history | diabetes_history | plan_type / punch_type | completion_status |
|----|--------|----------------|------------------|------------------------|-------------------|
| **需求 6.3/6.6 节** | male/female | yes/no | healthy/prediabetes/diagnosed | diet/exercise/other | completed/uncompleted |
| **数据库 CHECK 约束** | male/female | yes/no | healthy/prediabetes/diagnosed | diet/exercise/other | completed/uncompleted |
| **API 请求/响应契约** | male/female | yes/no | healthy/prediabetes/diagnosed | diet/exercise/other | completed/uncompleted |
| **TypeScript 类型** | 'male'/'female' | 'yes'/'no' | 'healthy'/'prediabetes'/'diagnosed' | 'diet'/'exercise'/'other' | 'completed'/'uncompleted' |
| **前端 UI 展示** | 男/女 | 有/无 | 健康/糖尿病前期/已确诊 | 饮食/运动/其他 | 已完成/未完成 |
...
- **下拉刷新/筛选器**：Punch.vue 的 `punch_type` 筛选 chip 按钮组，每个 chip 的 `data-value` 使用英文值（`diet`/`exercise`/`other`），按钮文本通过 `enumLabel` 显示中文。
...
```json
{
  ...
  "data": {
    ...
    "created_at": "2026-06-23T14:30:00"  // 详细设计扩展字段，需求 6.3 节未定义；保留用于前端展示预测时间
  }
}
```
...
> **枚举值规范（v13 修订，对齐需求 6.6 节）**：`punch_type` 枚举值 `diet`/`exercise`/`other`，`completion_status` 枚举值 `completed`/`uncompleted`，均为英文小写，与需求 6.6 节定义、DDL CHECK 约束、TypeScript 类型一致。前端 UI 渲染时通过 `enumLabel()` 映射为中文展示。
...
;
```
*注：数据库中 `punch_type` ('diet'/'exercise'/'other') 与 `completion_status` ('completed'/'uncompleted') 为英文，API 契约层同样使用英文枚举值（v13 修订后移除 mapper.js 转换层），后端直接透传英文值入库与返回；前端 UI 展示时通过 `enumLabel()` 函数映射为中文标签。*
```
...
> **响应结构对齐（v13 修订，对齐需求 6.7 节）**：`POST /api/articles/generate` 生成文章响应结构与 `GET /api/articles/:id` 详情结构保持一致，新增 `tags`、`summary`、`is_collected` 字段。新生成的文章 `is_collected` 固定为 `false`（当前用户尚未收藏）。`tags` 和 `summary` 由 Dify 文章生成工作流（5.3.3 节）在生成正文时同步产出，Express 端点将其与 `content` 一并写入 articles 表。
...
interface AuthActions {
  ...
  setAuth(newToken: string, newRole: 'user' | 'admin', user: User): void;  // v13 修订新增（对齐需求 4.10 节）：同时更新 token/role/userInfo 三个字段，用于跨浏览器标签页登录态同步
...
interface PunchRecord {
  ...
  punch_type: 'diet' | 'exercise' | 'other';  // 英文枚举值（v13 修订，对齐需求 6.6 节与 DDL）
...
interface PunchCreateRequest {
  plan_id: number;
  punch_type: 'diet' | 'exercise' | 'other';  // 英文枚举值（v13 修订，对齐需求 6.6 节）
...
interface PunchListParams extends PaginationParams {
  ...
  endDate?: string;
  punch_type?: 'diet' | 'exercise' | 'other';  // 英文枚举值（v13 修订）
}
...
interface AuthActions {
  ...
  setAuth(newToken: string, newRole: 'user' | 'admin', user: User): void;  // v13 修订新增（对齐需求 4.10 节）
...
|------|---------|---------|---------|-----------|
| blocking | 15s (连接) + 15s (读取) | 1 | 2s | 返回 504 AI_TIMEOUT |
...
async function executeHandler(req, res) {
  ...
  // 记录日志（v13 修订：admin_logs 表字段 admin_id 重命名为 operator_id，
  // insertAdminLog 函数内部 INSERT 语句使用 operator_id 列名，
  // 消除"admin_id 字段存储普通用户 user_id"的语义混淆）
