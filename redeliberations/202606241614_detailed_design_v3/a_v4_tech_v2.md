...
|-------|------|------|-----------|-------------|
| authStore | stores/authStore.ts | 登录态管理 | token: string\|null, user: User\|null, role: 'user'\|'admin'\|null, mustChangePassword: boolean | login(), logout(), setToken(), clearAuth() |
...
router.beforeEach(async (to, from, next) => {
  ...
  //    涉及AI生成内容的路由（医师对话/生活方案/风险预测/健康建议）首次访问前必须确认免责声明
  if (to.meta.requiresDisclaimer) {
    const { hasAcceptedDisclaimer, showDisclaimer } = useUI();
    if (!hasAcceptedDisclaimer()) {
      const accepted = await showDisclaimer();
      if (accepted) {
        // 同意后写入 localStorage，跨标签页持久化（storage 事件监听已在 App.vue 注册）
        localStorage.setItem('disclaimer_accepted', 'true');
      } else {
        // 用户拒绝，返回上一页；无上一页则回首页
        return next(from.fullPath && from.fullPath !== '/' ? false : '/home');
      }
    }
  }
...
- 在 `server/routes/punch.js` 的打卡端点中：落库前将 `punch_type`（'diet'/'exercise'/'other'）及 `completion_status`（'completed'/'uncompleted'）转换为英文，返回列表及分析响应前转换回中文。
...
| user_id | INTEGER | NOT NULL, FK | - | 用户ID |
| plan_type | TEXT | NOT NULL, CHECK(plan_type IN ('diet', 'exercise', 'other')) | - | 方案类型：diet=饮食, exercise=运动, other=其他。该三种类型方案项均可正常生成展示与打卡，与 `punch_in.punch_type` 的 CHECK 约束枚举维度保持一致 |
...
| user_id | INTEGER | NOT NULL, FK | - | 用户ID |
| plan_id | INTEGER | FK, ON DELETE SET NULL | NULL | 关联 life_plans 的方案项ID, 支撑依从性分析 |
| punch_time | TEXT | NOT NULL | datetime('now') | 打卡时间 |
| punch_type | TEXT | NOT NULL, CHECK(punch_type IN ('diet', 'exercise', 'other')) | - | 打卡类型：diet=饮食, exercise=运动, other=其他 |
...
```
*注：从数据库中查询出的 `gender` ('male'/'female') 与 `family_history` ('yes'/'no') 为英文，在 Express 控制器返回给前端响应前，需分别调用 `mapper.js` 的 `toChinese('gender', gender)` 和 `toChinese('family_history', family_history)` 转换为中文，以满足接口契约定义。*
...
;
```
*注：数据库中 `punch_type` ('diet'/'exercise'/'other') 与 `completion_status` ('completed'/'uncompleted') 为英文，后端落库前需调用 `mapper.js` 的 `toEnglish` 将参数转换后再存入；出库返回响应给前端前，需调用 `toChinese` 转换回中文，以满足接口契约定义。*
```
...
interface ChatState {
  ...
  adminConversationId: string | null;      // 管理员智能管理会话ID
...
interface ChatActions {
  ...
  clearChat(): void;
  getDoctorConversationId(doctorId: number): string | undefined;
  setDoctorConversationId(doctorId: number, id: string): void;
  getAssistantConversationId(): string | null;
  setAssistantConversationId(id: string | null): void;
  getAdminConversationId(): string | null;
...
```typescript
// stores/authStore.ts
interface AuthState { token: string | null; user: User | null; role: 'user' | 'admin' | null; mustChangePassword: boolean; }
...
flowchart TD
    ...
    O --> M
    N -->|event=message_end| P[chatStore.setAdminConversationId(conversation_id)]
...
**字段说明**：Dify 工作流同时输出英文枚举（`risk_level`、`diabetes_type`，供后端逻辑判断与 mapper.js 双向转换使用）与中文展示标签（`risk_level_label`、`matched_diabetes_type`，供前端直接渲染与历史记录查询提取）。`risk_level_detail` 为风险等级的中文详细描述文本，`suggestions` 为 3-5 条个性化建议数组。
...
| 分类 | 表名 | 用户操作允许条件 |
|------|------|-----------------|
| 用户私有表（含 user_id 列） | `user_risk_info`、`life_plans`、`life_advice`、`punch_in`、`article_collections`、`admin_logs` | WHERE 子句必须显式包含 `user_id = operatorId`（数值字面量或绑定参数等价物）；INSERT 的 VALUES 列表必须包含 `user_id` 且值为 `operatorId`；UPDATE/DELETE 的 WHERE 必须包含 `user_id = operatorId` |
...
const USER_SCOPED_TABLES = new Set([
  ...
  'punch_in', 'article_collections', 'admin_logs'
]);
...
function validateRowLevelPermission(sql, operatorId) {
  ...
  // 3. 用户私有表：强制 user_id = operatorId 约束
