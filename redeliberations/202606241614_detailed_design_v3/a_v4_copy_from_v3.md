...
;
```
*注：数据库中 `punch_type` ('diet'/'exercise') 与 `completion_status` ('completed'/'uncompleted') 为英文，后端落库前需调用 `mapper.js` 的 `toEnglish` 将参数转换后再存入；出库返回响应给前端前，需调用 `toChinese` 转换回中文，以满足接口契约定义。*
```
