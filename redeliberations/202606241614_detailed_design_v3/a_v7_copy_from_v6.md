...
// v14 修订：补充 role 独立 ref 变量声明与 login() 中 role 设置逻辑，对齐需求 6.1 节
export const useAuthStore = defineStore('auth', () => {
  const token = ref<string | null>(localStorage.getItem('token'));
  // v14 修订新增：role 作为独立 ref 变量，从 localStorage('role') 恢复，避免页面刷新时依赖 JWT 解码
  ...
  async function login(username: string, password: string) {
    ...
    // v14 修订：从响应顶层提取 role（与需求 6.1 节一致），同步写入 token/role/user 三个状态与 localStorage
  ...
  function logout() {
    // v15 修订说明：authStore.logout() 仅负责清除认证状态（token/role/user/mustChangePassword）
    // 完整的登出流程编排（包括中止 SSE 连接、清理对话会话、清除表单数据）在调用 authStore.logout() 之前执行
    // 详见 3.2.3 节"前端登出完整流程"段落和 4.3 节 Profile.vue 流程图登出分支
    token.value = null;
    role.value = null;
    user.value = null;
  ...
  return { token, role, user, mustChangePassword, login, logout };
});
```
...
**转换职责说明**：
- **写入路径**（前端 → API → Express → SQLite）：前端通过 `POST /api/articles/generate` 触发文章生成时，Express 调用 Dify `health-article-generator` 工作流，工作流输出 `tags: string[]` 数组（与 `summary`、`content` 一并产出），Express `articles.js` 路由处理器在执行 `INSERT INTO articles` 前对 `tags` 数组执行 `JSON.stringify(tags)` 转为字符串入库。`life_advice` 表由 AI 助手对话触发生成，写入逻辑同理。
...
| user_id | INTEGER | NOT NULL, FK | - | 用户ID |
| plan_id | INTEGER | FK, ON DELETE SET NULL | NULL | 关联 life_plans 的方案项ID, 支撑依从性分析。**v15 修订说明**：`plan_id` 在 `POST /api/punch` 接口层为必填（对齐需求 6.6 节），DDL 层允许 NULL 是为了支持 `ON DELETE SET NULL` 外键约束（当 `life_plans` 记录被删除时，`punch_in.plan_id` 自动设为 NULL 保留打卡历史）和管理员 Text2SQL 场景（管理员可能通过 `execute_SQL` 兜底工具直接 INSERT 打卡记录且不关联方案项）。应用层（Express `punch.js`）在 INSERT 前校验 `plan_id` 必填，DDL 层的 DEFAULT NULL 不影响接口契约的必填语义 |
...
1. **JSON 优先**：尝试 `JSON.parse(response.data.outputs.text)`，成功则直接按字段映射写入 `articles` 表
...
function dispatchParameterizedQuery(db, toolName, params, operatorId, operatorRole) {
  switch (toolName) {
    ...
    case 'query_user_profile': {
      // 普通用户仅能查询本人信息，管理员可查询任意用户
      const targetId = operatorRole === 'admin' ? (params.user_profile_id || operatorId) : operatorId;
      const rows = db.prepare(
        'SELECT id, username, role, avatar, created_at FROM users WHERE id = ?'
      ).all(targetId);
      return { rows };
    }
    case 'query_risk_info': {
      // 普通用户仅能查询本人风险记录，管理员可查询任意用户
      const targetUserId = operatorRole === 'admin' ? (params.target_user_id || operatorId) : operatorId;
      const rows = db.prepare(
    ...
    case 'query_punch_records': {
      const targetUserId = operatorRole === 'admin' ? (params.target_user_id || operatorId) : operatorId;
      const rows = db.prepare(
        'SELECT id, plan_id, punch_type, completion_status, remarks, punch_time FROM punch_in WHERE user_id = ? ORDER BY punch_time DESC LIMIT ?'
      ).all(targetUserId, params.limit || 30);
      return { rows };
    }
    case 'query_article_collections': {
      const targetUserId = operatorRole === 'admin' ? (params.target_user_id || operatorId) : operatorId;
      const rows = db.prepare(
        'SELECT ac.id, ac.article_id, a.title, a.category, ac.created_at FROM article_collections ac JOIN articles a ON ac.article_id = a.id WHERE ac.user_id = ? ORDER BY ac.created_at DESC'
      ).all(targetUserId);
      return { rows };
    }
    ...
    case 'query_doctor_list': {
      const rows = db.prepare(
        'SELECT id, name, department, title, description, avatar FROM doctor_information ORDER BY id'
      ).all();
      return { rows };
    }
    default:
